/**
 * Flask 框架适配器盲测扫描器（第 5 个框架适配）
 *
 * 对 generated-flask/ 每个项目：结构扫描（tools/extract_framework_flask.py）
 * → 规则判定（src/frameworks/flask-detector.ts）→ 与 gold.json 预期比对。
 * 口径：检出 = (rule, handler) 精确匹配金标。确定性管线（无 LLM）。
 *
 * Usage: npx ts-node blind-benchmark/scan-protocol-flask.ts
 */

import * as fs from "fs";
import * as path from "path";
import { execSync } from "child_process";
import { analyzeFlaskStructure } from "../src/frameworks/flask-detector";

const GEN_DIR = path.resolve(__dirname, "generated-flask");
const TOOL = path.resolve(__dirname, "..", "tools", "extract_framework_flask.py");
...
function main() {
  ...
  fs.writeFileSync(
    path.join(__dirname, "reports", "scan-protocol-flask-results.json"),
    JSON.stringify({
      generated: new Date().toISOString(),
      method: "extract_framework_flask.py + flask-detector（确定性，无 LLM）",
      summary: { tp, fp, fn, precision, recall },
      rows,
    }, null, 2)
  );
  console.log("报告 → blind-benchmark/reports/scan-protocol-flask-results.json");
}
