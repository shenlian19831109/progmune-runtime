...
> 生成日期：2026-08-23（v1.1：endState 检查上线后复测）  
> 结论：**Recall 100% / Precision 100% / 0 FP（40 处可测金标，含 endState 8 处）**

---

## 1. 概要

本基线给 Python 协议层（Auth / Resource Lifecycle）提供可测量的 P/R 数据——此前覆盖矩阵标注"无独立协议盲测基准"（⚠️ 行的判定依据之一）。

与 Python 缺陷检测盲测（v1：90 项目 / 729 gold，测源码级检测规则）不同，本基准隔离测量 **SSG 协议状态机**：per-function 调用序列经生产校验器（`src/trust/ssg-bridge.ts` 的 `validateSequenceWithSSG`）做 pre-state / invalidate / **endState（资源未释放）** 校验。风格变体沿用盲测方法论：同一违规分类学 × 结构变体（线性 / helper / 类方法 / 噪声穿插）。
...
| T5 | missing_cleanup | endState（序列末尾资源未释放，`open_file` 无 `close_file`） | 8 | ✅ |
...
1. **跨函数状态传播未实现**：per-function 序列验证不展开跨函数调用链——违规链必须落在单个函数体内才能检出（与 C 的 L3 同类边界）。本基准的植入全部为函数体内自包含，如实反映当前能力面。
2. **LLM 语义桥接层不在测量范围**：基准用规范协议名直构 steps（等价于生产路径 LLM 映射对规范名的输出），任意 API 名 → 协议名的桥接（`api-semantic-mapper`）需要独立基准（含 LLM，非确定性）。
3. **endState 检查语义边界**：仅资源生命周期命名空间（`RESOURCE_NAMESPACE_RE`：file/db/connection/socket/stream/io 等）且仅本序列新获取的状态——继承自命名空间初始状态的不算泄漏；auth/session 命名空间合法地以活跃会话结束，不做检查（与 planner 语义一致）。

## 5. 对覆盖矩阵的判定

覆盖矩阵 Python 列升级条件（Auth / Resource Lifecycle 的 ⚠️ → ✅）：

- [x] 有基准数据 + 可测 P/R（本基线：40 gold，100%/100%，0 FP）
- [x] endState 检查实现（T5 类违规 8/8 检出）
- [ ] 跨函数传播实现（或明确文档化为永久边界）
...
npx vitest run tests/python-protocol-benchmark.test.ts             # harness 回归（broken/clean/endState 锁定）
```
