...
|-----------|------|---------|------|
| Express | TS/JS | ⚠️ 启发式 | 路由提取 + 中间件分类 + 安全检查（代码串级；真实 FP 数据待补） |
| tRPC | TS/JS | ⚠️ 启发式 | API 合约规则（3 条），与 Express detector 交叉纠正（真实 FP 数据待补） |
| NestJS | TS/JS | ✅ 结构级（3.7.11） | 装饰器路由解析 + 全局 APP_GUARD 守卫识别 + @Public 豁免 + 守卫名认证分类（ThrottlerGuard≠认证）（合成金标 P=R=100%） |
| Next.js | TS/JS | ⚠️ 启发式（3.7.9） | App Router route.ts 写导出认证检查 + 认证 middleware；另有版本感知治理 |
| Fastify | TS/JS | ⚠️ 启发式（3.7.9） | 路由 preHandler/钩子认证分析（代码串级） |
| Koa | TS/JS | ⚠️ 启发式（3.7.12） | 路由/认证中间件链分析（app.use + 路由级中间件） |
| Hapi | TS/JS | ⚠️ 启发式（3.7.12） | 路由配置认证策略分析（auth 选项 + 显式 auth: false 检出） |
| 其余 TS 框架 | TS/JS | ⚠️ 基础别名 | 库别名覆盖，无结构分析 |
| Django | Python | ✅ 结构级（3.7.8） | URL 配置/CBV/DRF 结构适配：无保护 mutation 视图与 AllowAny 写端点标记（合成金标 P=R=100%；django-realworld 0 FP） |
| FastAPI | Python | ✅ 结构级（3.7.8） | 路由/认证依赖结构适配：写操作路由认证依赖检查 + 死认证方案标记（合成金标 P=R=100%；fastapi-realworld 0 FP） |
| Flask | Python | ✅ 结构级（3.7.9） | 路由/before_request 认证守卫分析（合成金标 P=R=100%） |
| Gin | Go | ⚠️ 启发式（3.7.13） | 路由/中间件链认证分析（r.POST 中间件 + Use/Group） |
| Fiber | Go | ⚠️ 启发式（3.7.13） | 路由/中间件链认证分析（app.Post 中间件 + Use） |
...
```
专用检测器        12 个 = 4 结构级（AST：NestJS/FastAPI/Django/Flask）
                  + 8 启发式（代码串：Express/tRPC/Fastify/Next.js/
                  Koa/Hapi/Gin/Fiber——真实 FP 数据待补为转正门槛）
基础别名覆盖      其余框架（Spring Boot 等）
```
...
Frameworks       12 专用（4 结构 + 8 启发）  Spring Boot 结构适配（需 Java）
TS P/R           100%/98.5%              稳定
