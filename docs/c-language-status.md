...
### 2026-08-28 annotation adoption tooling (`REALWORLD_C_V7.md`)

Annotation adoption is the lifecycle gate for the Beta label: `src/annotation-suggest.ts` (deterministic name-vocabulary heuristic — role/namespace/state-transition/template per candidate), `scripts/c-annotate.js --scan [--write]`, and an additive `evaluateTrust.annotationSuggestions` field (C-only). Acceptance on an unannotated uftpd copy: **gold recovery 4/4 role-correct** (check_user_pass=verify, handle_PASS=establish, do_RETR/do_STOR=guard); auto-apply of 7 suggestions is **equivalent to the hand-written gold** (0 SSG FP on real code, both seeded violations still precisely caught, same NEEDS_REVIEW 72). Three measured safety findings gate the auto-write defaults: (1) masking risk — annotating a flow function (body calls rule primitives) makes it an opaque primitive and hides its internal violations (measured: 2 seeded violations vanished) → maskRisk-flagged suggestions are never auto-written; (2) resource-lifecycle annotations auto-applied produce the cross-function-window FP class (measured: 28) → open/close skipped by default; (3) `new_session` (factory, not gate) vocabulary fix (measured: 4 FPs eliminated). Zero drift: Python blind v1.2 (64) + C app gold F1=95.7% + suites 98/98.
...
|------|----------|-----------|
| 2026-08-28 | 注解建议引擎 + 自动写入门控（采纳生死线落地） | 金标恢复 4/4 角色正确、自动应用与手写等价（0 FP + 违规精确定位）。**自动写入三门控全部实测**：掩蔽风险（establish 掩蔽植入违规 ×2 实测）永不自动写；open/close 资源注解跨窗口 FP（28 条实测）默认不写；new_session 类词汇误判（4 FP 实测）已收紧。 |
