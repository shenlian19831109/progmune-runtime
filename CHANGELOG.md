...
**一句话**：主 IR 循环的类方法分支**只调了 `computeMarkerCalls`（产 `__progmune_*` 语义
标记），从来没调过 `extractDirectCalls`（产真实调用名）**；而函数声明 / 箭头 / 包装箭头
三个分支都是两行齐全。后果是类方法的 `calls` 里永远只有标记、没有真实调用 ⇒ **协议状态
机在 OO 代码上拿不到输入**。

这不是新引入的回归。Claude 补充的历史事实：修 A1 时类方法提取循环里写死的就是
`calls: []`，`extractDirectCalls` 只对函数声明和箭头函数调用过，类方法从未接上。A1 只是
给方法条目挂标记时动过这个数组，基础的调用提取从来没补。**自该功能诞生之日起就空转**，
taintpath 十一族语料全是函数载体，从来没有东西逼它现形。

**顺带解释了三件悬案**：真实语料 `protocol=0`、引擎自扫「0 误报」、NestJS 语料过分干净
—— 裁判一直没拿到证据。

**修复**：类方法分支补上 `extractDirectCalls(m, mText)`（签名形参加 `MethodDeclaration`，
遍历逻辑无需改动），顺序与既有三个载体一致：先真实调用，再追加语义标记。

| 探针形态（18 条） | 修复前 | 修复后 |
|---|---|---|
| 类方法（同步 / async / this 成员 / 装饰器 / 构造函数注入） | 全空 | ✓ 全部提出 |
| static / private / 抽象类实现方法 / `this.a.b()` 链式 / 计算属性名 | 全空 | ✓ 全部提出 |
| 函数声明 / 箭头（基线对照） | ✓ | ✓ 不变 |

**三条不修**（探针确认后明确登记，避免下一轮重踩）：

1. **getter / setter / 构造函数体 / 对象字面量方法 / 类表达式** —— 这五种不是「calls 空」，
   而是**条目压根不在 IR 里**（载体未收集）。属另一类缺陷，风险量级不同（新增条目 = 新增
   检测面），不能与本轮混在一起，否则反向验证无法归因。
2. **嵌套箭头回调里的调用** —— `extractDirectCalls` 遇嵌套函数即 `traversal.skip()`。实测
   函数声明 / 箭头 / 类方法三者口径**完全一致**（都只提得到 `setTimeout`），是既有设计。
3. **计算属性名的名字解算** —— `C.[K]` 名字不可解是另一回事，不影响 calls。

### FP 观测池：把「裁判拿到证据」量化出来

修复后重扫（`fp-pool-scan.ts`，4 切片 / 197 文件）：

| 读数 | 修复前 | 修复后 |
|---|---|---|
| 空 calls 函数 | 140 / 197（**71%**） | 23 / 197（**12%**） |
| 其中类方法空转 | 126 | 9 |
| NestJS 切片 | 59 / 61 空 | 3 / 61 空 |
| Express 切片 | 76 / 113 空 | 15 / 113 空 |

**代价也量出来了**：项目级 safeguard 19 → 21，但 **perFunction 层面新增 18 条** —— 类方法
第一次有了 calls，规则第一次看见这些序列。逐条判定：**FP 15 / 弱真 3 / 真漏洞 0**。四条
规则侧校准线索已登记在 `fp-pool.json`（迁移与种子脚本应豁免、TypeORM query builder 链被
Input Validation 误判、内部工具方法不应被要求 Authorization、`validate` 已存在却仍报缺校验
⇒ 是词表不认 class-validator）。

上一轮 7 条人工判定里那条「无法判定」的 `UserController.login`（Token Security）**本轮变
成有效票**：有了 calls 才看得出它是标准 `jwt.sign` 签发 ⇒ FP。
...
| fr-007 / fr-016 | pre 5→post 0 / pre 7→post 0 **均维持** |
| 定向用例 | **241 passed / 6 文件**（新增 E1 组 14 条：10 正 2 正对照 2 负对照） |
| TS 盲测 | 111 项目 / perFunction **3201 条**：**LOST 0 / ADDED 0**，汇总完全一致 |
| 反向验证 | 三刀，残留 0（见下） |
| tsc | 零错误 |

**反向验证三刀**（`/tmp/rev_e1.py（原名 rev_c4k.py）`）：

| 刀 | 退回的旧行为 | 转红 |
|---|---|---|
| ① 类方法只留标记 | `const mCalls = computeMarkerCalls(...)` | **12 条**：正例 ①–⑩ + 负对照 ①② |
| ② 类方法不追加标记 | 摘掉 `mCalls.push(...computeMarkerCalls(...))` | 0 条（marker 由**调用方侧**补上） |
| ③ 同上（闸门视角） | 同 ② | 闸门仍 160/160 |

刀①归因**干净**：转红的 12 条全在类方法组，正对照 ⑪⑫（函数声明 / 箭头）仍绿。
刀②③顺带证明两件事：marker 链路有调用方侧兜底（不是单点）；**taintpath 语料里根本没有
类方法 ⇒ 闸门对本轮是空过**，真正的验收在 fp-pool 与定向用例。

### 方法学

- **R22 第三次应验**：负对照·② 一开始写错期望（以为类方法体内有 sink 就会有 marker），
  跑探针才确认 —— marker 只在**调用方把污点传进来**时才注入到被调用方。写 expect 之前
  先把判据抄进探针注释，这条规矩已经是第三次救我。
- **R20 的适用边界**（Claude 指出）：那三问（pre 有无校验 / 污点根是否在切片内 / fix 是否
  新增校验）只适用于**修复回归语料**，**别误用到 FP 观测池**——池的标准是「真实 + HTTP
  入口 + 文件操作」，不要求 CVE。两边管的方向不同（召回 vs 精度），标准本来就不同。

## [3.7.46] — 2026-09-21

### C4j：载体收口的最后一批 + 一处既有缺陷的回归门

C4i 的备忘留了四条候选。按 R17 先写探针复现，同时塞进七条「没人提过但看着差不多」的
形状 —— 三轮探针：24 条覆盖面 → 17 条**防修过头**的边界 → 6 条给默认导出补真 import。
真缺口六条；另有一条**不是这轮引入**的缺陷被探针顺手挖出来（见下）。

| 形状 | 旧行为 | 机制 |
|---|---|---|
| `typeof m === "string" \|\| n + ".md"` | 整条 helper 落选 | `typeof` 被当成**未知自由标识符** ⇒ 判据不通过 |
| `((n) => …) as Fn` / `satisfies` / `<Fn>…` | 不收 | 初值是**断言表达式**不是箭头；成员位同理 |
| `get mk() { return (n) => … }` | 不收 | getter 没有形参，`getFunctions()` 取不到 |
| `lvl3("a")("b")(k)` 三/四跳 | 不展 | 旧机制只吃**两跳** |
| `factory(".md").toPath(k)` | 不展 | 真正塑形的是**返回对象里的成员方法** |
| `export default (n) => …` | 收不到 | 匿名默认导出**没有名字**可登记 |

**精度侧一条：运算符关键字。** `typeof` / `void` / `in` / `instanceof` 是运算符不是自由
标识符，出现在 return 表达式里不该让整条 helper 落选。**只放行运算符** —— `this` /
`super` 不放行，它们指向宿主状态，放行等于给「返回值依赖 this」的假塑形开口子。

**召回侧五条。**

- **断言解包**：`unwrapFns` 解 `as` / `satisfies` / `<T>` 类型断言，**变量位、属性位、
  调用点 callee、返回位**都要用 —— `get mk() { return ((n) => …) as Fn }` 就是「返回位
  没解」漏出来的。
- **getter 返回的箭头**：getter 自己的形参表是空的，判据要落在**它返回出来的那一层**，
  与柯里化同一条路。
- **柯里化推广成「跳链」**：`CurrySpec` 从写死的「外层 + 一层内层」改成
  `{ outerParams, hops: CurryHop[], allParams, rets }`。`lvl3 = (a) => (b) => (n) => n+a+b`
  ⇒ `outerParams=[a]`、`hops=[call[b], call[n]]`；工厂形态 ⇒ `hops=[member toPath [n]]`。
  调用点从「两层实参代入」改成**逐跳消费**，跳数不设死上限（爬取有界 4 层），首跳实参
  同样被闭包捕获（所以 `lvl3(k)("a")("b")` 与 `lvl3("a")("b")(k)` 都要对）。
- **工厂形态 = 一个名字多条链**：按**成员名**选链（`makeMixed(".md").mix(k)` 走 mix 那条），
  脏链（返回常量 / 体内有 sink）单独剔掉，不许污染同名的另一条。
- **匿名默认导出**：没名字 ⇒ 用**导入方**起的本地名发布（`import toPath from "./helpers"`）。

### 探针挖出的一处既有缺陷（C4h-② 的洞）

C4h 把「同名成员唯一才登记」换成「全组确证」，但只把**通过判据**的实现算进组：判据不
成立的实现没进组 ⇒ 组看起来「全组确证」⇒ 成员调用位放行 ⇒ 误标。

```
同名三处 getter：无形参的（判据①不成立）/ 正常塑形 / 体内有 sink 的（判据③不成立）
后两处进组、第一处没进 ⇒ 组"干净" ⇒ .mk( 放行 ⇒ ONoParam.mk(k) 被误标
```

修法 `noteUnaccepted`：**失败的实现也记进组**（记 key + group，不进候选）。组一脏，整组
不放行。教训：**「全组」的分母必须包含被判据拒掉的那些**，否则「全组确证」会退化成
「通过者全体确证」—— 已升为方法学规则 **R19**。

### 又一条「探针期望写错」（第二次）

- **F9a**：探针忘了写 import。匿名默认导出的名字只能来自导入它的文件，两文件之间没有
  import 就无从登记 —— 期望本身不可达。补上真 import 后通过。
- **F9f**：期望 unmarked、实测 mark。查清后是**期望写错**不是误报：`(0, (n) => n + ".md")(k)`
  里那个**内联匿名箭头自己就是塑形函数**，与默认导出无关 —— 把 `helpers.ts` 删掉照样标。

⇒ 两条都不是机制错，是**预测错**。写期望前先问：这条标/不标，到底由**哪一个机制**决定。

### 三条「看着像缺口、这轮故意不修」的边界

探针里留着（F5 / F7 / A6 / F12），都退回保守侧：`switch` 分支与 `if` 早返回的返回值取
**并集**（常量分支和污点分支混在一起，判不出）；`x != null` 这类**比较式**左值不折；
标识符位「先声明后赋值」（`let f; f = (n) => …`）不收。这三条不是漏，是「判不出就退回
保守」的固有代价 —— 要收得让调用点折叠长出手臂（按分支路径分别代入），属另一轮的量级。

### 验收

| 门 | 结果 |
|---|---|
| taintpath 闸门 | **160/160**（新增 K 族 22 条：12 mark / 9 no-taint / 1 suppressed） |
| fr-007 真实语料 | pre 5 / post 0 **维持** |
| fr-016 真实语料 | pre 7 / post 0 **维持** |
| 定向用例 | 提取器定向用例 **227 passed / 6 个测试文件**（其中本文件新增 C4j 组 23 条：12 正 11 负，含两条回归·⑦） |
| TS 盲测 | 111 项目 / perFunction 口径 **3201 条**：**LOST 0 / ADDED 18**，全部来自新增的 K 族 —— 其中 12 条 `path_traversal` 是本该标的，6 条 `data_integrity`（写文件无审计）属新项目的无关类别；基线那 3183 条**零漂移零丢失** |
| tsc | 零错误 |

### 反向验证七刀（归因）

每刀只把一个机制退回**旧行为**（R18：不许 `return null` 逼停）：
...
| 5 | 工厂形态不登记 | emitFactory / emitFactoryMix |
| 6 | 摘掉匿名默认导出收集 | emitDefault |
| 7 | 成员组退回「判据失败不进组」 | 定向用例 回归·⑦ 两条 |

闸门其余 148 条、定向用例 C4j 组其余 21 条全程不动 ⇒ 七处互不相干，归因干净。

### 实现坑三条

1. **解包要解到位**：`returnedValueOf` 也得解断言 —— `get mk() { return ((n) => …) as Fn }`
   漏的就是这一处（"返回位没解"）。
2. **一条名字多条链时，脏链要单独剔掉**：`makeMixed` 的 `skip`（返回常量）不许把 `mix`
   那条链带下水，否则「张冠李戴」在链层面复现。
3. **`expandCurried` 的实参位置**：首跳实参落在外层形参的第一个，不是内层 —— 从「两层
   实参代入」改成「逐跳消费」时最容易写反。

## [3.7.45] — 2026-09-21

### C4i：又一批可判定分支 + 又一批 helper 载体

备忘里挂着两条（getter/computed 属性 helper、常量代入扩展到短路与模板串）。按 R17
先写探针复现，跑完 19 个形状：**备忘只覆盖了一半**，真缺口有六个，其中柯里化与 rest
两条备忘里根本没有。

| 形状 | 探针实测 | 结论 |
|---|---|---|
| 短路 `\|\|` / `&&` / `??` | `viaOr(k, "fixed.md")` 误标 | 真误报（精度侧） |
| 模板串内嵌三元 | `viaTpl(k, true)` 误标 | 真误报（精度侧） |
| 柯里化 `withExt(".md")(k)` | 不标 | 真漏报（备忘未列） |
| namespace 内函数声明 | `P.toPath(k)` 不标 | 真漏报 |
| rest 形参 `...parts` | `joinAll("out", k)` 不标 | 真漏报（备忘未列） |
| IIFE 定义 / 计算属性名 | `toPathIife(k)`、`Box.toPath(k)` 不标 | 真漏报 |
| getter（无形参） | 不标 | **不是缺口** —— 无形参 ⇒ 判据①本就不成立 |

**精度侧：可判定分支的扩展。** 三元之外，短路运算符与三元同形，只是判据从 cond 换成
**左操作数的真假**（`\|\|` 左真取左、`&&` 左真取右、`??` 左非空取左）。模板串的
`${...}` 是独立表达式，折叠要能进去 —— 现代代码里 `` `${flag ? "fixed" : n}.md` `` 比
裸三元更常见。判不出一律退回原串。

**召回侧：四个新载体。**

- **柯里化**：`withExt(ext)(n)` 真正塑形的是被返回出来的内层函数。收集时识别「return
  箭头」，判据落在内层；调用点把 `NAME(a)(b)` 按两层实参代入后**原地摊开**
  （⇒ `k + ".md"`），之后的污点检测直接看摊开后的串。
- **namespace**：旧收集走 `sf.getFunctions()`，实测取不到 namespace 里的函数 ⇒ 改遍历
  声明节点，只收顶层 / namespace 链上的（函数体内嵌套的作用域与调用点不同，不收）。
- **rest 形参**：它吃掉的是**一批**实参，不是一位。原先按位对齐会把第二位当成「越界」
  抹掉 ⇒ 漏报。现在 rest 位吃掉末尾所有位，且常量代入时跳过 rest 位。
- **IIFE / 计算属性名**：`(() => (n) => …)()` 的初值是调用不是箭头；`{ [KEY]: (n) => … }`
  的属性名只有在 KEY 是**模块级字面量常量**时才解 —— 解不出就不登记。

### 又一条「用例自己写错了」

初版把「属性名算不出来」的 `DynBox` 和同名可解的 `Box` 放在同一个项目里，期望
unmarked 却实测 mark。原因不是机制错：成员调用位 `.name(` 的匹配**本就与宿主无关**
（C4g 的保守策略），`Box.toPath` 登记之后 `DynBox.toPath` 必然被顺带命中。教训：写负
对照前先确认「不标」到底是因为哪一个机制不成立 —— 这条已写进语料注释。
...
| fr-007 真实语料 | pre 5 / post 0 **维持** |
| fr-016 真实语料 | pre 7 / post 0 **维持** |
| 定向用例 | 提取器定向用例 **204 passed / 6 个测试文件**（其中本文件新增 C4i 组 26 条：11 正 15 负） |
| TS 盲测 | 110 项目 / perFunction 口径 **3183 条**：**LOST 0 / ADDED 18**，全部来自新增的 J 族 —— 其中 12 条 `path_traversal` 是本该标的，6 条 `data_integrity`（写文件无审计）属新项目的无关类别；基线那 3165 条**零漂移零丢失** |
| tsc | 零错误 |
...
| 刀 | 改动 | 转红 |
|---|---|---|
| 1 | `topLevelLogical` 恒返回 null（关短路折叠） | `emitOrTakeLeft` / `emitAndTakeLeft` / `emitNullishTakeLeft`（no-taint 转误标） |
| 2 | `foldTemplateSubsts` 恒返回原串 | `emitTplConst`（转误标） |
| 3 | `currySpecOf` 恒返回 null（柯里化不登记） | `emitCurrySecond` / `emitCurryFirst` / `emitCurryFn`（转漏报） |
| 4 | `moduleQualifier` 恒返回 null（namespace 不发限定名） | `emitNsPath`（转漏报） |
| 5 | `paramIndexOf` 退回按位（rest 位截断） | `emitRestJoin`（转漏报） |
| 6 | 不解 IIFE（`functionLikeOf` 的 CallExpression 分支） | `emitIifePath`（转漏报） |
| 7 | 不解计算属性名（`propertyNameOf` 的 computed 分支） | `emitComputed`（转漏报） |

七刀各自只动自己那一组，没有交叉。第 2 刀和第 5 刀第一次都**打偏了**（详见下），
打偏的刀会连累无关用例 —— 已升为方法学规则 **R18**。

### 实现坑三条

1. **`substituteParams` 把模板串整段跳过了** —— 折叠逻辑写对了也没用，`${flag ? …}`
   里的 flag 根本没被代进去。模板串要按 `${}` 逐段递归代入，字面量部分照抄。
2. **ts-morph 的两层包装**：`namespace P { … }` 里函数的父节点是 **ModuleBlock**
   （ModuleDeclaration 在上一层）；`(() => (n) => …)()` 里被括号包住的**不止**内层
   箭头，连被调用的 callee 都是 `ParenthesizedExpression`。两处都得脱。
3. **模板串里写注释会提前终结模板串**（生成器里 `${}`、反引号都要转义）—— 报错信息
   指向的是文件末尾，很容易误判成别处语法错。

### 反向验证的两刀打偏（R18）

反向验证不是"随便让一个函数失效"就行，退回必须是**旧行为**：

- 第 5 刀第一次写成 `paramIndexOf` 返回 `null` —— rest 位不是"退回按位"，而是整个
  失去对应形参 ⇒ 所有 helper 的实参被抹光 ⇒ **40 条漏报**。正确写法是 `return argIndex`。
- 第 2 刀第一次写成 `foldTemplateSubsts` 返回 `null` —— 调用方靠 `tpl !== s` 判断
  "这一层有没有做事"，返回 null 会被当成"已折叠" ⇒ 连带 11 条无关用例转红。

⇒ R18：**反向验证的刀要退回旧行为，不能随便 return null**；刀的转红清单必须落在
自己那一组，一旦出现跨组转红，先怀疑刀打偏了，而不是"机制有耦合"。

## [3.7.44] — 2026-09-20

### C4h：名录之外的三处收口

helper 名录这轮的起点是三个备忘候选，动工前先写探针复现（R12），结果只有两个是真
缺口，外加一个**探针顺手挖出来的既有缺陷**：
...
| ——（非备忘项） | 方法名唯一但判据不成立时，成员调用位照样进名录 | **既有缺陷**，`r.lookup(k)` 误标 |

**① 调用点常量代入。** 名录里的依赖集是**跨调用点**的并集 —— 对所有实参形状都成立
的最宽结论，收不住个案：
...
哪些形参活着；判不出（实参不是常量、cond 代入后算不出真假）就一律退回名录结论 ——
保守侧，认不出就不收精度。

**② 同名成员多处。** 旧规则要求方法名全项目唯一才登记 `.method(` 成员调用位。唯一性
只是「不可能认错」的**充分条件**，不是必要条件：同名两处都被确证为纯塑形时，同样
不可能认错。换成「全组确证」之后依赖集按形参位取并集（任一实现在某位会流出，那位
就算会流出）。

同一处顺带补上一个坑：候选登记改成 `key / name / group` 三段，**确证之后**才发布到
成员名录 —— 旧代码在确证之前就把裸名登记进去，于是
`class Reg { lookup(n) { return registry[n]; } }`（registry 是模块级非字面量对象 ⇒
判据③不成立）之后的 `r.lookup(k)` 被当成塑形 ⇒ 误标。

**③ 属性承载的箭头。** `const Util = { toPath: (n) => … }` 与 `Util.toPath = (n) => …`
既不是方法声明也不是变量声明，旧名录一片都收不到。现在与方法同表登记（限定名 +
同组确证发布裸名）。

### R13 第二次的变种：假通过也会发生在**定向用例**里

C4g 组的静态类方法用例原本叫 `Renderer.stat(k)` —— `stat` 就在 fs sink 名单里，调用点
被 sink 兜底直接标中，**那条用例一直在假通过**（删掉整段方法收集它照样绿）。改名
`toFile` 后才真的在测 helper 传播。R16 原只约束盲测语料命名，本轮起同样适用于定向
用例：写 fixture 前先 grep `TS_FILE_SINK_NAMES`。
...
| fr-007 真实语料 | pre 5 / post 0 **维持** |
| fr-016 真实语料 | pre 7 / post 0 **维持** |
| 定向用例 | 提取器定向用例 **178 passed / 6 个测试文件**（其中本文件 102 passed，新增 C4h 组 21 条：12 正 9 负） |
| TS 盲测 | 109 项目 / perFunction 口径 **3165 条**：**LOST 0 / ADDED 11**，全部是新增 I 族里那 11 条本该标的 path_traversal；基线那 3152 条**零漂移零丢失**（另 2 条 `data_integrity` 属新项目的无关类别） |
| tsc | 零错误 |
...
| 刀 | 改动 | 转红 |
|---|---|---|
| 1 | 关掉调用点折叠（`callSiteKeepFlags` 退回名录 deps） | `emitFlagTrue` / `emitPickMiss` / `emitFlagNested` / `emitWrapFlagTrue`（4 条由 no-taint 转误标） |
| 2 | 同名合并退回「全项目唯一」 | `emitSameNameA` / `emitSameNameB`（转漏报） |
| 3 | 摘掉属性箭头收集（(d)(e) 两个分支） | `emitPropArrow` / `emitNestedPropArrow` / `emitPatchedArrow`（转漏报） |
| 4 | 成员名退回「确证之前就登记」 | `emitNonShaperMember` / `emitSameNameRejected`（转误标 —— 那个缺陷就是这么被探针逮到的） |

四刀各自只动自己那一组，没有交叉：归因是干净的。

### 实现坑两条

1. **整条括号会让 cond 切不干净**：return 表达式常被写成 `=> (flag ? a : b)`，在第一
   个顶层 `?` 处切开得到的前缀 `((true)` 括号不配对 ⇒ 判不出真假 ⇒ 折叠静默失效。
   要先脱掉包住整串的括号（`stripOuterParens`）。
2. **代入时别给字面量加括号**：`(true)` 会让 `maskNonDependentArgsOnce` 把实参串误判
   成嵌套调用（它的判据是「实参里还有 `(`」），于是内层 helper 永远轮不到被处理 ——
   两跳那条（`wrapFlag(k, true)`）就卡在这儿。

## [3.7.43] — 2026-09-20

### C4g：helper 换个载体（对象字面量方法 / 类方法 / 解构形参）

helper 不一定是函数，也可能是方法：
...
C4e 时刻意没做这一路（按裸名放行会让别的对象上的同名方法被误认成塑形，R11）。
这一轮用**限定名 + 唯一性**解决：

- 宿主名能确定时登记 `Owner.method`，调用点 `Util.toPath(k)` 直接对上；
- 实例方法的主要写法是 `r.inst(k)`（宿主是变量，限定名对不上），所以该方法名在
  **全项目唯一**时额外允许 `.inst(` 这种成员调用位匹配 —— 唯一性是 R11 的替代品，
  不存在同名方法就不可能张冠李戴。同名方法出现两次时一律不登记，宁可漏报。

另补**解构形参**：`function f({ name }) {…}` 的 `getName()` 给的是整个绑定模式
而不是 `name`，要从 BindingElement 里逐个抽出来，否则这类 helper 一个都认不出。

R12 第四次生效：备忘里列的「依赖分析深层形态」（三元返回、默认参、多 return）
探针实测**本来就是通的**，真漏的只有解构形参。

### 语料踩到的坑：helper 名字不能撞 sink 名单

H 族第一版把静态方法写成 `Renderer.stat(k)`，闸门全绿；反向验证摘掉方法收集之后
它**仍然标**。一查：`stat` 就在 `TS_FILE_SINK_NAMES` 里，调用点被 sink 兜底直接
命中，与 helper 传播毫无关系 —— 实测 `Other.stat(k)`（接收者根本没定义）、以及类里
压根没有 `stat` 方法，照样标。

这是 R13 那条旁路的第二次出现，而且更阴：它让正例「标对了」，但标对的原因是另一
回事。改名为 `toFile` 之后归因才干净。已登记为 **R16**。

### 验收

| 门 | 结果 |
|---|---|
| taintpath 闸门 | **92/92**（新增 H 族 11 条：7 mark / 3 no-taint / 1 suppressed） |
| fr-007 真实语料 | pre 5 / post 0 **维持** |
| fr-016 真实语料 | pre 7 / post 0 **维持** —— 放宽召回后守卫侧仍无新增误报 |
| 定向用例 | **171 passed / 7 个测试文件**（新增 C4g 组 7 条：4 正 3 负） |
| TS 盲测 | 108 项目，path_traversal **LOST 0 / ADDED 27**（H 7 + E 9 + F 6 + G 4 + D 1），基线那 104 个项目**零漂移零丢失** |
| tsc | 零错误 |
...
刀 4 顺带证明 C4f 的收窄对新载体同样生效 —— `Drop.fixed(k)` 不是空转对照。

### 刻意不做：常量实参代入

`viaTernaryDrop(n, flag) { return flag ? "fixed.md" : n + ".md"; }` 在
`viaTernaryDrop(k, true)` 下走的是常量分支、形参不流出，现在仍会被判污染。要收这条
得在调用点做常量传播（把实参字面量代入 return 再重算依赖），是另一个量级的改动，
且代入出错的代价是漏报。登记为已知缺口。
