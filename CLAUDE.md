...
- **启发式（代码串模式）**：Express（路由+中间件分类）、tRPC（3 条合约规则）、Fastify（preHandler/钩子）、Next.js（App Router route.ts 导出）、Koa/Hapi/Gin/Fiber（路由中间件链/配置认证）——单测覆盖。**真实 FP 数据点进度：2/8**——Express（2026-09-02，`REALWORLD_FRAMEWORK_FP_V1.md`）：realworld 20 flags 协议级 TP 0/20（FP 侧证据）；Fastify（同日，`REALWORLD_FRAMEWORK_FP_V2.md`）：avanelli fastify-realworld 真实 20 路由全不可见（object-form `server.route({})` + plugin 注入 + `onRequest:[server.authenticate]` 认证），0/20 路由 + 无感反证成立（摘掉 POST /articles 认证后输出不变）——**recall 侧证据，转正需结构性重写**。两者皆维持「启发式 ⚠️」有据；Express 语料还产出并修复 1 个真缺陷（cors() 误分类 security_header → NO_HELMET 漏报，已修 + 回归 ×2）。剩余 6 个（tRPC/Next.js/Koa/Hapi/Gin/Fiber）待补
- 转正门槛：每个启发式探测器补一个真实项目 FP 数据点（C 的 real-corpus 方法论延伸）后才可升级结构级标签；Express 转正 = 清单 4 项做完在本语料重测 TP/FP 变化；Fastify 转正 = object-form 路由/plugin 注入/onRequest+decorator 认证/register 链 4 项重写后 20 路由全见 + 变异实验有反应
