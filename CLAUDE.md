...
**Framework adapters: 8/13 with structural analysis** (Express, NestJS, FastAPI, Django, Flask, Fastify, Next.js), plus a tRPC detector (3 rules) and 5/13 with library aliases. Express detector does route extraction + middleware classification + security checks. NestJS detector (3.7.11 completed) parses decorators via ts-morph + global APP_GUARD providers + @Public exemption + guard auth-classification. **Python trio (3.7.8/3.7.9)**: structure extracted by Python AST scanners (`tools/extract_framework_py.py` / `extract_framework_django.py` / `extract_framework_flask.py` — routes, Depends/Security injection, urlconf/CBV/DRF permission_classes, before_request guards), rules decided TS-side (`src/frameworks/{fastapi,django,flask}-detector.ts`: unprotected mutation routes, AllowAny write endpoints, dead auth schemes). **TS trio (3.7.9)**: Fastify (`fastify-detector.ts` — route preHandler/hook code-string analysis) and Next.js (`nextjs-detector.ts` — App Router route.ts mutation exports + auth middleware). All feed into the trust engine pipeline (python-only or TS-file-scan, best-effort, additive `*Coverage` fields). Remaining frameworks (Spring Boot, Go frameworks, some TS frameworks) have basic alias coverage but no structural analysis.
