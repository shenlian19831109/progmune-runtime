...
- **启发式（代码串模式）**：Express（路由+中间件分类）、tRPC（3 条合约规则）、Fastify（preHandler/钩子）、Next.js（App Router route.ts 导出）、Koa/Hapi/Gin/Fiber（路由中间件链/配置认证）——单测覆盖。**真实 FP 数据点进度：6/8**——Express（`…V1.md`）：20 flags 协议级 TP 0/20（FP 形态失配）；Fastify（`…V2.md`）：真实 20 路由全不可见 + 无感反证（recall 侧）；Koa（`…V3.md`）：300 字符窗口跨路由串扰 → 单点摘 auth 无感（分类器缺陷侧）；tRPC（`…V4.md`）：netflx-web 19 过程只见 7、mutation 0/8 可见——链正则不跨嵌套括号（0 FP 但 coverage 受限）；Next.js（`…V5.md`，同 netflx-web）：webhook 签名认证不在会话词表 → 1 flag = 1 FP（支付应用系统性误报）；Hapi（`…V6.md`）：gothinkster hapi v16 realworld **0/38 文件进门**——gate/hasHapi 只认 @hapi-scoped 与 `Hapi.server`，v16 `require('hapi')` 插件形态全灭；即便进门，声明式数组路由 + `config.auth` 嵌套也不匹配（最外层 gate 失明）。已修复：Express cors→security_header。剩余 2 个（Gin/Fiber，Go 族）待补
- 转正门槛：每个启发式探测器补一个真实项目 FP 数据点（C 的 real-corpus 方法论延伸）后才可升级结构级标签；Express 转正 = 清单 4 项做完重测；Fastify 转正 = object-form/plugin/onRequest+decorator/register 重写后 20 路由全见 + 变异有反应；Koa 转正 = 窗口按路由边界截断 + 幻影路由限定接收者后重测；tRPC 转正 = 括号感知链解析 + lastIndex 修复后 19 过程全见 + 摘 input 有反应；Next.js 转正 = 认证词表补 webhook 签名/裸 auth()/clerk + webhooks 豁免后重测；Hapi 转正 = 门兼容 `require('hapi')` + 数组/声明式路由 + config.auth 嵌套解析后重测
