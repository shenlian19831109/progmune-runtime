...
**Framework adapters: 12 dedicated detectors — 4 structural (AST-based) + 8 heuristic (code-string), plus 5/13 with library aliases.** 实现分层仅为内部工程注释；对外按**证据档位**（2026-09-02 评审定稿）：
> 🟡 合成验证（Synthetic）→ ✅ **真实语料验证**（≥1 真实生产语料 0 协议级 FP + 摘保护反证通过）→ 🏭 生产级（企业 POC/采纳，暂无）。**框架适配 12/12 现为 ✅**（2026-09-02 全量收官）。边界如实注明：Express main.ts 剩 NO_HELMET 加固缺口（非协议）；NestJS 语料检出 1 真实 TP（DELETE /users/:slug，= 敏感性证明）；jiotv_go 2 flags 能力令牌形态（DecryptURLParam 校验）；gofiber recipes demo flags 语境噪声（无认证意图示例）。语料：gothinkster 家族 RealWorld（express/koa/hapi/gin/nestjs…）+ 真实产品（netflx-web/journalist/jiotv_go）+ 官方 recipes；考核工具 `npm run audit:realworld`（docs/REALWORLD_METHODOLOGY.md）。
