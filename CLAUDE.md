...
- **结构级（AST 解析）**：NestJS（ts-morph 装饰器 + APP_GUARD + @Public 豁免）、FastAPI/Django/Flask（Python AST 扫描器 tools/extract_framework_{py,django,flask}.py → 路由/依赖注入/urlconf/权限类）。**真实语料考核（2026-09-02，`REALWORLD_STRUCTURAL_V1-V4.md`）——1/4 通过**：FastAPI（nsidnev realworld 2.4k★）0 issues 真实 + 对缺失敏感（现代 Depends 惯用法 ✓；遗留 user 词假保护 FN 风险）；Django（gothinkster realworld）urlconf 直连视图通过（变异有反应）但 **DRF ViewSet + DefaultRouter（router.register）路由不可见**——核心写面未被检查；NestJS（lujakob realworld 2.6k★，Nest5 中间件时代）guard 单一惯用法失明——configure/forRoutes 中间件保护 0 反应、23 issues 全 FP、摘保护无感；Flask（gothinkster realworld）AUTH_WORDS 缺 "jwt" → @jwt_required 不可见 → 10/10 FP。**定论：结构级 ≠ 免疫真实失明——词表精度、注册机制覆盖、惯用法模型是与启发式共享的软肋**
