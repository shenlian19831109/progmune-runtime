...
| Java | 🔶 框架层真实语料验证（SSG 建设中） | 3.7.17：`extract-ir-java.ts` 提取器注册（LANGUAGE_EXTRACTORS + engine 分派）；Spring 路由覆盖适配器（`src/frameworks/spring-detector.ts`）——gothinkster spring-boot-realworld 1581★：19 路由/12 mutation 全解析、0 issues、anyRequest 兜底翻转反证 10 重现（`REALWORLD_SPRING_V1.md`）；audit:realworld 支持 --framework spring。核心 SSG 注解驱动协议行待建 |
