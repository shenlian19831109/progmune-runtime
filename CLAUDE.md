...
| Java | 🔶 框架层真实语料验证 ✅ + 核心协议行引擎化（合成金标） | 3.7.17：`extract-ir-java.ts` 提取器注册（LANGUAGE_EXTRACTORS + engine 分派）+ Spring 路由覆盖适配器（`src/frameworks/spring-detector.ts`）——gothinkster spring-boot-realworld 1581★：19 路由/12 mutation 全解析、0 issues、anyRequest 兜底翻转反证 10 重现（`REALWORLD_SPRING_V1.md`）。3.7.20-21：**SSG 注解驱动协议行引擎化**（`// @protocol` 方法级注解 → 注解合并 → SSG 序列验证全自动，同 C 路径）——v1 token（authenticate→AUTHENTICATED 先于 use）、v2 auth/register（hashPassword 先于 storeUser）、v3 resource 管理（openFile/closeFile invalidate + 泄漏端）三族共 8 正负例引擎回归锁定（docs/java-language-status.md）；真实语料协议行金标待标注 |
