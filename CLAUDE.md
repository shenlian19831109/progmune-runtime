...
**Framework adapters: 4/13 with structural analysis** (Express, NestJS, FastAPI, Django), plus a tRPC detector (3 rules) and 5/13 with library aliases. Express detector does route extraction + middleware classification + security checks. NestJS detector parses decorators (@Controller, @UseGuards, @UsePipes) via ts-morph. **FastAPI + Django (3.7.8)**: structure extracted by Python AST scanners (`tools/extract_framework_py.py` / `tools/extract_framework_django.py` — routes, Depends/Security injection, urlconf/CBV/DRF permission_classes), rules decided TS-side (`src/frameworks/fastapi-detector.ts` / `django-detector.ts`: unprotected mutation routes, AllowAny write endpoints, dead auth schemes). Both feed into the trust engine pipeline (python-only, best-effort, additive `fastapiCoverage`/`djangoCoverage`). Remaining frameworks (Next.js, Fastify, Flask, etc.) have basic alias coverage but no structural analysis.
