...
import type { GoalConstraint } from "./runtime-types";
...
export interface StructuredGoal {
  /** The protocol namespace this goal targets. */
  protocol: string;
  /** The starting state(s) of the protocol. */
  initialState: string[];
  /** The desired end state(s). */
  targetState: string[];
  /** Soft constraints (weighted preferences, not hard rules). */
  constraints: GoalConstraint[];
  /** Raw LLM parse or fallback method. */
  source: "llm" | "keyword";
}
...
function loadKnownProtocols(): Map<string, { pre_states: string[]; post_states: string[] }[]> {
  const protoPath = path.resolve(__dirname, "..", "protocols.json");
  if (!fs.existsSync(protoPath)) return new Map();

  try {
    const raw = JSON.parse(fs.readFileSync(protoPath, "utf-8"));
    const map = new Map<string, { pre_states: string[]; post_states: string[] }[]>();
    for (const [fn, rule] of Object.entries(raw) as [string, any][]) {
...
async function parseWithLLM(goal: string): Promise<StructuredGoal | null> {
  try {
    const response = await generate(
      FEWSHOT_PROMPT + `Goal: "${goal}"\nOutput:`
    );
...
const KEYWORD_MAP: [RegExp, Partial<StructuredGoal>][] = [
...
function parseByKeyword(goal: string): StructuredGoal {
...
function extractConstraintsFromGoal(goal: string): GoalConstraint[] {
 ...
 */
export async function parseGoal(goal: string): Promise<StructuredGoal> {
  // Try LLM
  try {
 ...
 */
export function parseGoalSync(goal: string): StructuredGoal {
  return parseByKeyword(goal);
}
