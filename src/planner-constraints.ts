...
import { mineRules, toProtocolEntries } from "./rule-miner";

interface PlannerConstraint {
  /** Pattern to match in function name/purpose */
  pattern: string;
  /** Score multiplier (0-1): 0 = blacklist, 0.5 = deprioritize, 1 = no effect */
  penalty: number;
  /** Human-readable reason */
  reason: string;
  /** Confidence from the mined rule */
  confidence: number;
}
 ...
 */
export function deriveConstraints(): PlannerConstraint[] {
...
): { multiplier: number; matchedRules: string[] } {
  let multiplier = 1.0;
 ...
 */
export function formatConstraints(constraints: PlannerConstraint[]): string {
...
let _constraints: PlannerConstraint[] | null = null;
export function getConstraints(): PlannerConstraint[] {
  if (!_constraints) _constraints = deriveConstraints();
  return _constraints;
}
export function clearConstraintsCache(): void { _constraints = null; }
