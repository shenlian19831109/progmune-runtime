...
// 错误注入定义
interface MaliciousTest {
  name: string;
  modify: (actions: any[]) => any[];
}

async function runTest(name: string, actions: any[]) {
  console.log(`\n🧪 测试: ${name}`);
  // 校验
  const results = actions.map((a: any) => validateAction(a));
  const valid = results.every((r: any) => r.valid);
  if (!valid) {
    console.log("  ❌ 校验器拦截:");
    results.forEach((r: any, i: number) => { if (!r.valid) console.log(`    动作${i}: ${r.errors}`); });
    return;
  }
  // 发射代码
  ...
  console.log("  生成的代码:\n" + code.split("\n").map(l => "    " + l).join("\n"));
  // 编译运行
  const execResult = runAndCheck(code);
...
async function main() {
  ...
  // 4. 错误注入测试集
  const maliciousTests: MaliciousTest[] = [
    {
      ...
      modify: (actions) => {
        ...
        for (const a of copy) {
          if (a.kind === "call" && a.function === "verifyPassword") {
            const hashArg = a.args.find((x: any) => x.name === "hash");
            if (hashArg) hashArg.type = "Token";
          }
        }
        return copy;
      }
    ...
    },
    {
      ...
      modify: (actions) => {
        ...
        for (const a of copy) {
          if (a.kind === "call" && a.function === "verifyPassword") {
            const plainArg = a.args.find((x: any) => x.name === "plain");
            if (plainArg) plainArg.type = "number";
          }
        }
        return copy;
      }
