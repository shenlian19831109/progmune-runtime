...
import { extractCallPairs, extractProtocolFromFiles, InferredRule, rulesToAnnotationMap, ExtractionResult } from "./protocol-extractor";
import { scanRepository, compareRules } from "./repo-evaluator";
...
import { runFailureAttribution, computeErrorBudget } from "./evaluation-campaign";
...
export function inferStateName(fn: string, role: "pre" | "post" | "invalidate"): string {
...
// ═══════════════════════════════════════════════════════════════
// Enhanced Rule Inference
// ═══════════════════════════════════════════════════════════════

export interface EnhancedRule extends InferredRule {
  domain: string;
  stateNames: { pre: string[]; post: string[]; invalidate: string[] };
}

/**
 * Enhanced protocol rule inference with intelligent state names.
 */
export function inferEnhancedRules(
  pairs: { from: string; to: string; file: string; line: number }[],
...
function inferDomain(fn: string): string {
  const lower = fn.toLowerCase();
  for (const [dom, keywords] of Object.entries(DOMAIN_KEYWORDS)) {
...
export interface FoundationExtractionResult {
  protocol: string;
  rules: EnhancedRule[];
  states: string[];
  sourceFiles: string[];
  ...
  // Comparison against hand-written baseline
  handWrittenOverlap: number;
  novelRulesCount: number;
}
 ...
 */
export function extractProtocolFoundation(
  repoPath: string,
  protocolName?: string,
  maxFiles: number = 200,
  minFrequency: number = 3
  ...
  const allPairs: { from: string; to: string; file: string; line: number }[] = [];

  for (const fp of files) {
    try {
      const code = fs.readFileSync(fp, "utf-8");
      allPairs.push(...extractCallPairs(code, fp));
    } catch { /* skip */ }
  }
  ...
  for (const r of rules) {
    ...
    if (r.invalidate) for (const s of r.invalidate) states.add(s);
  }

  // Compare against hand-written baseline
  const defs = loadDefaultProtocolDefinitions();
  const handWrittenFns = new Set<string>();
  for (const p of defs) for (const fn of p.rules.keys()) handWrittenFns.add(fn);

  const extractedFns = new Set(rules.map(r => r.function));
  let overlap = 0;
  for (const fn of extractedFns) {
    if (handWrittenFns.has(fn)) overlap++;
  }
 ...
 */
export function enhancedRulesToMap(rules: EnhancedRule[]): Map<string, StateAnnotation> {
...
// ═══════════════════════════════════════════════════════════════
// Discovery Impact Measurement
// ═══════════════════════════════════════════════════════════════

export interface DiscoveryImpact {
  baseline: { discoveryRate: number; top3Rate: number; missingCandidate: number };
  extracted?: { discoveryRate: number; top3Rate: number; missingCandidate: number };
  improvement?: { discoveryGain: number; top3Gain: number };
  rulesAdded: number;
}
 ...
 */
export async function measureDiscoveryImpact(
  extractedRules: Map<string, StateAnnotation>
): Promise<DiscoveryImpact> {
  // Baseline: current hand-written rules
  const baselineAttr = await runFailureAttribution();
  const baselineBudget = computeErrorBudget(baselineAttr);
  const baselineDiscovery = computeDiscoveryMetrics(baselineAttr);
...
export function printFoundationReport(result: FoundationExtractionResult, impact?: DiscoveryImpact): void {
  console.log("\n╔════════════════════════════════════════════════════╗");
