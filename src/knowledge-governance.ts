...
import { synthesizeTransitions, augmentRulesWithInferences, InferredTransition } from "./transition-synthesizer";
...
// ═══════════════════════════════════════════════════════════════
// P3.21: Inference Validation
// ═══════════════════════════════════════════════════════════════

export type ValidationStatus = "proposed" | "verified" | "rejected";

export interface ValidationResult {
  benchmarkSupport: number;
  trajectorySupport: number;
  contradictionCount: number;
  status: ValidationStatus;
}

export interface ValidatedTransition extends InferredTransition {
 ...
 */
export function validateInferences(
  inferences: InferredTransition[],
  trajectoryCount: number = 0
): ValidatedTransition[] {
  return inferences.map(inf => {
    ...
    if (contradictionCount > VALIDATION_THRESHOLDS.contradictionMax) {
      ...
      benchmarkSupport >= VALIDATION_THRESHOLDS.benchmarkMin &&
...
export type PatchStatus = "proposed" | "approved" | "rejected" | "rolled_back";

export interface KnowledgePatch {
  id: string;
  generatedBy: string;
  protocol: string;
  change: string;               // "open_file → flush_file"
  fromState: string;
  toState: string;
  confidence: number;
  ...
  approvedAt?: string;
  rolledBackAt?: string;
  /** Benchmark metrics at time of approval. */
  approvalMetrics?: {
    top1Before: number;
    top1After: number;
    top3Before: number;
...
const KNOWLEDGE_DIR = path.resolve(
  ...
  ".progmune_corpus", "knowledge"
);

export class KnowledgePatchStore {
  private patches: KnowledgePatch[] = [];
  ...
  propose(inference: ValidatedTransition): KnowledgePatch {
  ...
  approve(id: string, metrics: { top1Before: number; top1After: number; top3Before: number; top3After: number }): boolean {
  ...
  rollback(id: string): boolean {
    const patch = this.patches.find(p => p.id === id);
    if (!patch || patch.status !== "approved") return false;
  ...
  get all(): readonly KnowledgePatch[] { return this.patches; }
  get size(): number { return this.patches.length; }
...
// ═══════════════════════════════════════════════════════════════
// P3.23: Knowledge Regression Test
// ═══════════════════════════════════════════════════════════════
 ...
 */
export function regressionTestPatch(
  patch: KnowledgePatch,
  baseRules: Map<string, StateAnnotation>,
  testCases: { currentState: string[]; targetState: string[] }[],
  store: KnowledgePatchStore
): RegressionResult {
  // Top-1/Top-3 BEFORE patch (using base rules only)
  let top1Before = 0;
  let top3Before = 0;
  for (const tc of testCases) {
    const result = searchFrontier(baseRules, tc.currentState, tc.targetState, 8);
    if (result.found && result.cost <= 1) top1Before++;
    if (result.found && result.cost <= 3) top3Before++;
  }
  ...
  let top3After = 0;
  for (const tc of testCases) {
    const result = searchFrontier(augmented, tc.currentState, tc.targetState, 8);
    if (result.found && result.cost <= 1) top1After++;
    if (result.found && result.cost <= 3) top3After++;
  }
 ...
 */
export function autoApprovePatch(
  patch: KnowledgePatch,
  baseRules: Map<string, StateAnnotation>,
  testCases: { currentState: string[]; targetState: string[] }[],
  store: KnowledgePatchStore
...
export function runKnowledgeGovernance(
  ...
  // 1. Synthesize
  const inferences = synthesizeTransitions(failures);
...
export function printGovernanceReport(report: GovernanceReport): void {
  console.log("\n╔════════════════════════════════════════════════════╗");
