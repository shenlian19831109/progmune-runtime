...
function doubleBlindScramble(sequences: string[][]): string[][] {
  // Step 1: Collect all unique function names
  const allNames = new Set<string>();
  for (const seq of sequences) for (const fn of seq) allNames.add(fn);
...
describe("P8.1.1 Double-Blind State Inference Validation", () => {
  it("DOUBLE-BLIND: identical topology with scrambled random names → identical fingerprint", () => {
    ...
    for (const name of origNames) {
      expect(scramNames.has(`F_${String(name).padStart(4, "0")}`) || scramNames.has(name)).toBe(false);
    }

    const orig = extractStateFingerprint(inferStateMachine(FILE_PROTOCOL));
    const scram = extractStateFingerprint(inferStateMachine(scrambled));
    ...
    const sim = stateFingerprintSimilarity(orig, scram);
    ...
    expect(orig.stateCount).toBe(scram.stateCount);
    expect(orig.transitionCount).toBe(scram.transitionCount);
  });

  it("DB-LEVEL: database protocol survives double-blind at repo level", () => {
    const scrambled = doubleBlindScramble(DB_PROTOCOL);
    const orig = extractStateFingerprint(inferStateMachine(DB_PROTOCOL));
    const scram = extractStateFingerprint(inferStateMachine(scrambled));
  ...
  it("AUTH-LEVEL: auth lifecycle survives double-blind", () => {
    const scrambled = doubleBlindScramble(AUTH_LIFECYCLE);
    const orig = extractStateFingerprint(inferStateMachine(AUTH_LIFECYCLE));
    const scram = extractStateFingerprint(inferStateMachine(scrambled));
  ...
  it("CROSS-REPO-LEVEL: all 5 known repos survive double-blind", () => {
    const results: { repo: string; similarity: number }[] = [];

    for (const [repo, seqs] of Object.entries(CROSS_REPO)) {
      const scrambled = doubleBlindScramble(seqs);
      const orig = extractStateFingerprint(inferStateMachine(seqs));
      const scram = extractStateFingerprint(inferStateMachine(scrambled));
      const sim = stateFingerprintSimilarity(orig, scram);
  ...
  it("DISCRIMINATION-AFTER-SCRAMBLE: different repos remain distinguishable", () => {
    ...
    const redisFp = extractStateFingerprint(inferStateMachine(redisScram));
    const pgFp = extractStateFingerprint(inferStateMachine(pgScram));

    const sim = stateFingerprintSimilarity(redisFp, pgFp);
