...
export type RepairStrategy = "insert" | "replace" | "reorder";

export interface RepairProposal {
  id: string;                      // unique proposal ID
  violationIndex: number;          // which transition index is problematic
  reason: string;                  // why repair is needed
  strategy: RepairStrategy;
  insertBefore?: number;           // strategy=insert: insert new action before this index
  replacement?: StateTransition;   // strategy=replace: corrected transition
  newOrder?: number[];             // strategy=reorder: proposed index permutation
  proposedActions: Action[];       // corresponding Action sequence
  confidence: number;              // 0.0 – 1.0
  explanation: string;             // human-readable explanation
}

/** Summary of all repair proposals for a session/ledger. */
export interface RepairSummary {
  sessionId?: string;
  totalViolations: number;
  proposals: RepairProposal[];
  /** The minimal set of proposals needed to fix all violations (deduplicated). */
  minimalFixSet: RepairProposal[];
}
...
export function suggestProtocolRepair(
  ...
  if (rejection.fixPath && rejection.fixPath.length > 0) {
    ...
    for (let i = 0; i < rejection.fixPath.length; i++) {
      ...
      if (def) {
        const args = (def.params || []).map((p: any, j: number) => ({
          name: p.name || `p${j}`,
          type: p.type || "any",
          value: "",
        }));
...
export function suggestInvariantRepair(
  ...
  if (violation.invariant === "before-consistency") {
    ...
    const correctedTransition: StateTransition = {
      ...t,
...
function suggestGenericRepair(
  ...
  switch (violation.violatedConstraint) {
    case "symbol_existence": {
      ...
      if (bestMatch) {
        const def = ir.find((f: any) => f.name === bestMatch);
        const args = (def?.params || []).map((p: any, j: number) => ({
          name: p.name || `p${j}`,
          type: p.type || "any",
          value: "",
  ...
  switch (proposal.strategy) {
    ...
    case "replace": {
      if (proposal.replacement && proposal.violationIndex < currentLedger.length) {
        branch.transitions = currentLedger.map((t, i) =>
          i === proposal.violationIndex ? proposal.replacement! : t
        );
      } else {
        branch.transitions = [...currentLedger];
      }
...
export function validateProposal(
  ...
  switch (proposal.strategy) {
    case "replace":
      proposedLedger = currentLedger.map((t, i) =>
        i === proposal.violationIndex && proposal.replacement ? proposal.replacement : t
      );
      break;
    case "reorder":
...
function tryParseRejection(v: ConstraintViolation): SSGRejection | null {
  try {
    if (v.description) {
