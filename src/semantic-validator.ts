...
let irCache: IRFunction[] | null = null;
...
export function checkSemantic(intent: string, actions: Action[]): { valid: boolean; errors: string[] } {
  const ir = loadContracts();
  const errors: string[] = [];
  const callActions = actions.filter((a): a is Action & { kind: "call" } => a.kind === "call");

  for (const action of callActions) {
    const funcDef = ir.find(f => f.name === action.function);
    if (!funcDef || !funcDef.contracts) continue;

    for (const contract of funcDef.contracts) {
      if (contract.when_intent && !matchIntent(intent, contract.when_intent)) continue;

      switch (contract.type) {
        case "require_param": {
          const arg = action.args?.find((a: any) => a.name === contract.param);
