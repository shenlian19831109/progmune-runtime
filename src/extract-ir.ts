 ...
 */
function augmentPathTraversalMarkers(
  project: Project,
  funcs: FunctionInfo[],
  absRoot: string
): void {
  ...
  const mark = (name: string, file: string) => {
  ...
  for (const sf of project.getSourceFiles()) {
    ...
    sf.forEachDescendant((node) => {
      ...
      if (hasTaintedSinkCall(text, tainted)) {
        const nodeName = Node.isArrowFunction(node) ? undefined : node.getName();
        if (Node.isFunctionDeclaration(node) && nodeName) {
          mark(nodeName, relPath);
        } else if (Node.isMethodDeclaration(node) && nodeName) {
          const cls = node.getParent();
          if (Node.isClassDeclaration(cls) && cls.getName()) {
...
function augmentSsfrMarkers(
  ...
  for (const sf of project.getSourceFiles()) {
    ...
    sf.forEachDescendant((node) => {
      ...
      const text = node.getText();
      if (text.length > 12000) return; // 巨型函数跳过（性能 + 噪声）
      // 预过滤：必须有 fetch sink
      sinkRe.lastIndex = 0;
      if (!sinkRe.test(text)) return;
      // 守卫证据：函数内有 SSRF 防护词汇 → 抑制
      if (SSRF_GUARD_EVIDENCE.test(text)) return;

      // URL 形参污点
      const urlParams = Node.isArrowFunction(node)
        ? []
        : collectUrlParamNames(node.getParameters().map((p: any) => ({ name: p.getName() })));
      // request 污点（Web 形态）
      ...
      while ((m = sinkRe.exec(text)) !== null) {
        ...
        if (taint.test(after)) { flagged = true; break; }
      }
      if (!flagged) return;
      ...
      if (Node.isFunctionDeclaration(node) && nodeName) {
        ...
        const cls = node.getParent();
        if (Node.isClassDeclaration(cls) && cls.getName()) {
          const entry = funcs.find(
            (x) => x.name === `${cls.getName()}.${nodeName}` && x.file === relPath
          );
          if (entry) {
            entry.calls = entry.calls || [];
            if (!entry.calls.includes("__progmune_ssrf_user_url__")) {
