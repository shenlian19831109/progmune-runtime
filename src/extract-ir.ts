...
function pureShaperFunctionNames(project: Project): Set<string> {
  const sinkRe = tsSinkCallRegex();
  const cands: Array<{ name: string; params: Set<string>; rets: string[] }> = [];
  ...
  const shapers = new Set<string>();
  for (let round = 0; round < 2; round++) {
    let added = false;
    for (const c of cands) {
      if (shapers.has(c.name)) continue;
...
function collectTaintedNames(text: string, shapers?: Set<string>): Set<string> {
 ...
 */
function taintedViaShaper(text: string, tainted: Set<string>, shapers?: Set<string>): boolean {
  // C4b：项目自有「纯塑形」helper 的调用也算子（如 getFileNamePath(dir, name, ext)）。
  // 见 pureShaperFunctionNames —— 只认三条判据全部成立的函数，认不出就不传播。
  const localShaperRe =
    shapers && shapers.size > 0
