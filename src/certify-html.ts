...
import type { Certificate } from "./certify";
import { evaluatePolicy } from "./policy/engine";
import type { PolicyContext } from "./policy/engine";

// ── Types ──

interface CertificatePlus extends Certificate {
...
export function generateCertificateHTML(
  filePath: string,
  options: { author?: string; reviewer?: string } = {}
): string {
  ...
  try {
    ...
    const opts: any = {};
    if (options.author) opts.author = { id: options.author, name: options.author.split("@")[0], role: "developer" };
    if (options.reviewer) opts.reviewers = [{ id: options.reviewer, name: options.reviewer.split("@")[0], role: "reviewer" }];
    let acct;
    try { acct = buildAccountabilityChain(cert.sessionId, opts); } catch { /* no data */ }
...
function renderHTML(cert: CertificatePlus): string {
...
function hashCertificate(cert: CertificatePlus): string {
