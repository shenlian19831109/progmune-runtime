...
export interface DiscoveredProtocol {
  /** Name assigned to the discovered protocol. */
  name: string;
  /** The state machine fingerprint. */
  fingerprint: StateMachineFingerprint;
  /** Synthesized protocol rules (name-free). */
  rules: { function: string; pre_states: string[]; post_states: string[]; invalidate?: string[] }[];
  /** Sequences that formed this cluster. */
  prototype: string[];
  /** Which known protocol it most resembles (if any). */
  closestKnown?: string;
  /** Similarity to the closest known protocol. */
  matchConfidence: number;
}

export interface ZeroShotResult {
  /** Number of protocols discovered. */
  discoveredCount: number;
  /** Discovered protocols. */
  discovered: DiscoveredProtocol[];
  /** How many test defects were repaired using discovered knowledge. */
  repairSuccess: number;
  /** Total test defects. */
  repairTotal: number;
  /** Zero-shot repair success rate. */
  repairRate: number;
}
 ...
 */
export function buildKnownFingerprintLibrary(): Map<string, StateMachineFingerprint> {
  const defs = loadDefaultProtocolDefinitions();
  const library = new Map<string, StateMachineFingerprint>();

  for (const p of defs) {
    if (p.rules.size === 0) continue;
    library.set(p.name, extractStateMachine(p.rules));
  }
 ...
 */
export function extractUnknownRepoSequences(repoName: string): string[][] {
  return CROSS_REPO_SEQUENCES[repoName] || [];
}
 ...
 */
export function discoverProtocolsFromSequences(
  sequences: string[][],
  repoName: string,
  knownLibrary?: Map<string, StateMachineFingerprint>
): DiscoveredProtocol[] {
  ...
  const discovered: DiscoveredProtocol[] = [];
  ...
  for (const synth of synthesized) {
    ...
    const fingerprint = extractStateMachine(ruleMap);

    // 4. Match against known library
    let closestKnown: string | undefined;
    let matchConfidence = 0;

    if (knownLibrary && knownLibrary.size > 0) {
      for (const [name, knownFp] of knownLibrary) {
        const comp = compareStateMachines(fingerprint, knownFp);
...
// ═══════════════════════════════════════════════════════════════
// Step 4: Zero-Shot Repair
// ═══════════════════════════════════════════════════════════════

export interface DefectCase {
  /** The broken (incomplete) sequence. */
  broken: string[];
  /** The expected complete sequence. */
  expected: string[];
 ...
 */
export function evaluateZeroShotRepair(
  discovered: DiscoveredProtocol[],
  defectCases: DefectCase[]
): { success: number; total: number; details: string[] } {
  let success = 0;
  ...
  for (const defect of defectCases) {
    ...
    for (const proto of discovered) {
      ...
      const nameMap = new Map<string, string>(); // defect name → rule name
      for (const dfn of defectFns) {
        ...
        for (const rfn of ruleFns) {
          ...
          if (dist < bestDist) { bestDist = dist; bestFn = rfn; }
        }
        if (bestFn) nameMap.set(dfn, bestFn);
      }
      ...
      const currentStates = new Set<string>(["INIT"]);
      for (const fn of mappedBroken) {
        const rule = rules.get(fn);
        if (!rule) continue;
        if (!rule.pre_states.every(s => currentStates.has(s)) && rule.pre_states.length > 0) continue;
        if (rule.invalidate) rule.invalidate.forEach(s => currentStates.delete(s));
        for (const s of rule.post_states) currentStates.add(s);
      }

      // Frontier exploration for completion paths
      const paths = exploreFrontier(rules, [...currentStates], 20, 8);
 ...
 */
export function runZeroShotDiscovery(
  trainRepos: string[],
  testRepo: string,
  defectCases: DefectCase[]
): ZeroShotResult {
  // 1. Build known fingerprint library from train repos
  const knownLibrary = new Map<string, StateMachineFingerprint>();
...
export function printZeroShotResult(result: ZeroShotResult): void {
  console.log("\n╔════════════════════════════════════════════════════╗");
