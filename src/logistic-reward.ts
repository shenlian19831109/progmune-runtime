...
import type { CandidateFeatures } from "./repair-types";
...
// ═══════════════════════════════════════════════════════════════
// Extended Features (7-d for logistic regression)
// ═══════════════════════════════════════════════════════════════

export interface LogisticFeatureVector {
  protocolSafety: number;
  historicalSuccessRate: number;
  normActionCount: number;    // actionCount / maxActions
  latencyCost: number;
  auditability: number;
  acceptanceRate: number;      // from TelemetryIndex
  executionSuccessRate: number; // from TelemetryIndex
}

export interface TrainingSample {
  features: LogisticFeatureVector;
  label: number;  // 1 = accepted+executed, 0 = rejected|executionFailed
}
...
export interface LogisticModelConfig {
  ...
  batchSize: number;
  minSamples: number;         // minimum training samples before model activates
  convergenceThreshold: number; // stop if loss change < this
}

const DEFAULT_CONFIG: LogisticModelConfig = {
  learningRate: 0.01,
  epochs: 200,
  l2Lambda: 0.001,
  batchSize: 32,
...
function featureVecToArray(f: LogisticFeatureVector): number[] {
  return [
    f.protocolSafety,
...
export class LogisticRewardModel {
  public readonly weights: number[];  // 7 weights
  public readonly bias: number;
  private readonly config: LogisticModelConfig;
  private trained: boolean;
  private trainedSamples: number;
  private loss: number;

  constructor(weights?: number[], bias?: number, config?: Partial<LogisticModelConfig>) {
  ...
  score(features: CandidateFeatures, telemetryStats?: { acceptanceRate: number; executionSuccessRate: number }): number {
    const ts = telemetryStats || { acceptanceRate: 0.5, executionSuccessRate: 0.5 };
    const maxActions = Math.max(features.actionCount, 8);

    const fv: LogisticFeatureVector = {
  ...
  get finalLoss(): number { return this.loss; }
  ...
  static train(
    ...
    config?: Partial<LogisticModelConfig>
  ): LogisticRewardModel {
  ...
  private static collectSamples(telemetry: PlannerTelemetry): TrainingSample[] {
  ...
  private static normalizeSamples(samples: TrainingSample[]): TrainingSample[] {
  ...
  exportWeights(): { weights: number[]; bias: number; trainedSamples: number; loss: number } {
    return {
  ...
  static importWeights(data: { weights: number[]; bias: number; trainedSamples: number; loss: number }, config?: Partial<LogisticModelConfig>): LogisticRewardModel {
...
export function compareModels(
  ...
  if (model.isTrained) {
    ...
    for (const d of testDecisions) {
      ...
      const prediction = model.score(
        { protocolSafety: 0.8, historicalSuccessRate: 0.5, actionCount: sel.actions.length, latencyCost: 0.5, auditability: 0.5, corpusEvidence: 0, source: "protocol" },
        { acceptanceRate, executionSuccessRate }
      );
      if ((prediction >= 0.5 ? 1 : 0) === label) correct++;
