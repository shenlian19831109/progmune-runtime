...
export function assertDeltaConsistency(transition: StateTransition): void {
  ...
  const fromSnapshot = (snap: Record<string, string[]>) => {
  ...
  const toSnapshot = (stateMap: Map<string, Set<string>>) => {
  ...
  for (const s of transition.acquired) {
    ...
    if (!deltaMap.has(ns)) deltaMap.set(ns, new Set());
...
export function assertTransitionOrder(ledger: StateTransition[]): void {
...
export function assertLedgerInvariants(
  ...
  for (const t of ledger) {
    if (t.valid) assertDeltaConsistency(t);
  }
  if (expectedRuleHash) {
