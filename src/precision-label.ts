...
function loadLabels(repoPath: string): LabelData {
...
function saveLabels(repoPath: string, data: LabelData): void {
...
function extractSequences(repoPath: string): Array<{ index: number; calls: string[]; source: string }> {
  const ir = extractIR(repoPath);
  const seqs: Array<{ index: number; calls: string[]; source: string }> = [];

  // Strategy 1: Functions with protocol annotations
  for (const fn of ir) {
    if (fn.protocol) {
      const calls: string[] = [];
      if (fn.protocol.pre_states) calls.push(...fn.protocol.pre_states);
      calls.push(fn.name);
      if (fn.protocol.post_states) calls.push(...fn.protocol.post_states);
      if (calls.length >= 2) {
  ...
  if (seqs.length < 20) {
    for (const fn of ir) {
      if (fn.calls && fn.calls.length >= 2) {
        const relevantCalls = fn.calls.filter(
          (c: string) => ir.some((f: any) => f.name === c)
        );
...
async function main() {
  ...
  for (const seq of sequences) {
    ...
    if (data.labels[seq.index] !== undefined) continue;
