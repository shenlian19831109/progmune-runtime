...
import {
  extractProtocolFromFiles, extractProtocol, ExtractionResult, InferredRule,
  rulesToAnnotationMap, extractCallPairs,
} from "./protocol-extractor";
...
export function scanRepository(repoPath: string, maxFiles: number = 500): string[] {
  const files: string[] = [];
...
export interface RuleComparison {
  ...
  matchedRules: string[];       // extracted rules that match ground truth
  novelRules: string[];         // extracted rules NOT in ground truth
  missedRules: string[];        // ground truth rules NOT found by extraction
  precision: number;            // matched / totalExtracted
  recall: number;               // matched / totalGroundTruth
  f1: number;
}
...
export function compareRules(
  extracted: InferredRule[],
  groundTruth: Map<string, StateAnnotation>
): RuleComparison {
...
export interface DefectDetectionResult {
  ...
  /** Violations: call sequences that violate inferred rules. */
  violations: { file: string; line: number; missing: string; detail: string }[];
}
...
export function detectDefects(
  ...
  const violations: DefectDetectionResult["violations"] = [];
  let totalPairs = 0;

  for (const fp of files) {
    try {
      const code = fs.readFileSync(fp, "utf-8");
      const pairs = extractCallPairs(code, fp);
...
export interface RepoEvalReport {
  repo: string;
  filesScanned: number;
...
export async function evaluateRepository(
  ...
  maxFiles: number = 100
): Promise<RepoEvalReport> {
  // 1. Extract rules
  const files = scanRepository(repoPath, maxFiles);
  const extraction = extractProtocolFromFiles(
  ...
  // 3. Detect defects
  const extractedMap = rulesToAnnotationMap(extraction.rules.slice(0, 30));
...
export function printRepoEvalReport(report: RepoEvalReport): void {
  console.log("\n╔════════════════════════════════════════════════════╗");
