...
export interface CapabilityGap {
  /** What capability is missing (e.g., "oauth_authentication") */
  capability: string;
  /** Which part of the intent requires it */
  requiredBy: string;
  /** Confidence that this gap is real (0-1) */
  confidence: number;
  /** Suggested function names if we were to build this */
  suggestedNames: string[];
}

export interface GapAnalysis {
  /** Capabilities found in IR that match the intent */
  satisfied: string[];
  /** Capabilities needed but not found */
  gaps: CapabilityGap[];
  /** Overall coverage: how much of the intent can be satisfied */
  coverage: number;
}
 ...
 */
function extractCapabilities(intent: string): string[] {
  ...
  for (const pat of patterns) {
    ...
    if (matches) caps.push(...matches.map(m => m.toLowerCase()));
  }
 ...
 */
export function gapAnalysis(intent: string, ir: FunctionInfo[]): GapAnalysis {
...
export function formatGapReport(analysis: GapAnalysis): string {
  ...
  if (analysis.satisfied.length > 0) {
    ...
    for (const s of analysis.satisfied) lines.push(`  - ${s}`);
    lines.push("");
  }
