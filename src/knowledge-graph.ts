...
import type { KnowledgeUnit, KnowledgeUnitRelation, UnitRelationship } from "./protocol-knowledge";
...
interface KnowledgeGraph {
  nodes: GraphNode[];
  edges: GraphEdge[];
  adjacency: Record<string, string[]>;   // node → [neighbors]
  inDegree: Record<string, number>;       // how many nodes point TO this node
  outDegree: Record<string, number>;      // how many nodes point FROM this node
}

// Cross-protocol relations that go beyond existing unit.relations
const CROSS_PROTOCOL_EDGES: GraphEdge[] = [
  // Transport layer
...
export function buildKnowledgeGraph(): KnowledgeGraph {
  ...
  const nodes: GraphNode[] = kb.units.map(u => ({
  ...
  const edges: GraphEdge[] = [...CROSS_PROTOCOL_EDGES];
  ...
  for (const n of nodes) { adj[n.id] = []; inDeg[n.id] = 0; outDeg[n.id] = 0; }
...
export function getDependents(graph: KnowledgeGraph, nodeId: string): GraphNode[] {
  const ids = graph.edges.filter(e => e.to === nodeId).map(e => e.from);
  return graph.nodes.filter(n => ids.includes(n.id));
}

export function getDependencies(graph: KnowledgeGraph, nodeId: string): GraphNode[] {
...
function formatTree(graph: KnowledgeGraph): string {
  ...
  function renderNode(id: string, prefix: string, isLast: boolean) {
  ...
  for (const r of appRoots) {
    if (!visited.has(r.id)) renderNode(r.id, "", true);
  }

  return l.join("\n") + "\n";
}

function formatMatrix(graph: KnowledgeGraph): string {
