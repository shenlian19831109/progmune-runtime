...
interface AttemptRecord {
  ...
  errorDetail: string;
  actionSequence: { kind: string; function?: string }[];
  ssgFixPath?: string[];
...
function narrateRejection(a: AttemptRecord): string {
...
function narrateSuccess(actions: { kind: string; function?: string }[]): string {
  const names = actions
...
function formatActionSequence(actions: { kind: string; function?: string }[]): string {
  return actions
    .filter(a => a.kind === "call" && a.function)
    .map(a => C_(a.function! + "()"))
    .join(`  ${D("→")}  `);
...
function formatSessionTimeline(session: ReturnType<typeof getAllSessions>[0]): string {
  const width = 66;
  ...
  for (let i = 0; i < session.attempts.length; i++) {
    ...
    const narration = narrateRejection(a as AttemptRecord);
...
function formatSessionSummary(sessions: ReturnType<typeof getAllSessions>): string {
...
function formatGenomeSummary(): string {
  const g = getFailureGenome();
...
function barChart(count: number, total: number): string {
  if (total === 0) return D("▏".repeat(0));
  const maxBars = 20;
  const n = Math.round((count / total) * maxBars);
  if (n === 0 && count > 0) return Y("▏");
  if (n === 0) return D("▏".repeat(0));
  return Y("█".repeat(n)) + D("░".repeat(maxBars - n));
}

// ── Learned patterns ──

function aclBadge(level: string): string {
  switch (level) {
...
function formatLearnedPatterns(): string {
...
function loadProtocols(): FunctionProtocol[] {
...
function formatStateTransitionPath(actions: { kind: string; function?: string }[]): string {
  ...
  for (const step of trace) {
    ...
    if (lost.length > 0) delta += ` ${R("-" + lost.join(",-"))}`;
...
function formatStateTransitions(session: ReturnType<typeof getAllSessions>[0]): string {
  const width = 66;
  ...
  for (const a of session.attempts) {
    if (a.violatedSVL === "SVL-4" && a.ssgFixPath) {
      ...
      const actions = (a.actionSequence || []).filter((x: any) => x.function).map((x: any) => x.function).join(" → ");
...
function svlLabel(svl: string): string {
  switch (svl) {
...
function describeSVLLayer(svl: string): string {
  switch (svl) {
...
function formatReplayHeader(session: ReturnType<typeof getAllSessions>[0]): string {
  const lines: string[] = [];
  const w = 62;
...
function formatActionList(actions: { kind: string; function?: string }[]): string {
  if (!actions || actions.length === 0) return D("(empty sequence)");
  return actions
    .filter(a => a.kind === "call" && a.function)
    .map(a => `  ${C_(a.function! + "()")}`)
    .join("\n");
...
function formatAnomalyReport(a: AttemptRecord): string {
  const lines: string[] = [];
  const svl = a.violatedSVL;
...
function formatImmuneResponse(a: AttemptRecord): string {
  const lines: string[] = [];
...
function formatReplayProgress(current: number, total: number): string {
  const filled = "█".repeat(current);
  const empty = "░".repeat(total - current);
  return `  ${D("[" + filled + empty + "]")} ${current}/${total}`;
}

async function replaySession(session: ReturnType<typeof getAllSessions>[0]) {
  clearScreen();
  console.log(formatReplayHeader(session));
  await sleep(1200);

  const attempts = session.attempts as AttemptRecord[];
  ...
  if (session.resolved && session.successfulAlternative) {
    const actions = (session.successfulAlternative as { kind: string; function?: string }[]) || [];
...
function formatSessionValidation(session: ReturnType<typeof getAllSessions>[0]): string {
  const lines: string[] = [];
  const C_ = (s: string) => `${C.cyan}${s}${C.reset}`;
  ...
  const allCalledFns = new Set<string>();
  for (const a of session.attempts) {
    for (const act of (a.actionSequence || [])) {
      if (act.function) allCalledFns.add(act.function);
    }
  }
  ...
  // Load snapshotted IR
  let snapshotFns: Set<string> | null = null;
  if (session.snapshotId) {
    const snap = loadSnapshot(session.snapshotId);
    if (snap) {
  ...
  // Load current IR
  let currentFns: Set<string> | null = null;
  try {
