...
import {
  traceSearch, decomposeMissingCandidate, DecomposedMissingCandidate,
  computeDiscoveryReport, printDiscoveryReport, DiscoveryResult,
  evaluateOffPolicy, printReplayEvaluation,
  learnProtocolBridges,
} from "./discovery-trace";
...
describe("Search Trace", () => {
  it("traces a frontier search and classifies dead ends", () => {
    const rules = makeRules();
    // Expect "flush_file" which doesn't exist → missing_action
    const trace = traceSearch(rules, ["FILE_OPEN"], [], ["close_file", "flush_file"], "frontier", "safely write file");

    expect(trace.expandedNodes.length).toBeGreaterThan(0);
    expect(trace.deadEnds.length).toBeGreaterThan(0);
    // flush_file is not in the rules → missing_action
    expect(trace.deadEnds.some(d => d.reason === "missing_action")).toBe(true);
  });

  it("decomposes missing candidates into root causes", () => {
    const rules = makeRules();
    const traces = [
      traceSearch(rules, ["FILE_OPEN"], [], ["close_file", "flush_file"], "frontier", "write file"),
      traceSearch(rules, ["UNAUTHENTICATED"], ["SESSION_ACTIVE"], ["verify_password", "generate_jwt", "create_session"], "frontier", "auth"),
    ];

    const decomposition = decomposeMissingCandidate(traces);
    expect(decomposition.total).toBe(2);
    expect(decomposition.missingAction).toBeGreaterThan(0);
  });
});

describe("Discovery Benchmark", () => {
  it("computes three-tier metrics", () => {
    ...
    const report = computeDiscoveryReport(results);
    expect(report.discoveryRate).toBe(0.75); // 3/4
    expect(report.top3Rate).toBe(0.5);       // 2/4
    expect(report.top1Rate).toBe(0.25);      // 1/4
...
describe("Counterfactual Replay", () => {
  it("evaluates off-policy improvement", () => {
    const decisions = [
    ...
    function newGen(goal: string, _protocol: string): string[][] {
      if (goal.includes("safely write")) return [["open_file", "write_file", "close_file"]];
      if (goal.includes("authenticate")) return [["verify_password", "generate_jwt", "create_session"]];
      return [];
    }

    const eval_ = evaluateOffPolicy(decisions, newGen);

    // Baseline: 0/2 (neither matches)
    expect(eval_.baselineRate).toBe(0);
    // New: 2/2 (both match)
    expect(eval_.newRate).toBe(1);
    expect(eval_.improvement).toBe(true);

    printReplayEvaluation(eval_);
  });
});

describe("Protocol Bridge Learning", () => {
  it("learns bridges from cross-protocol failures", () => {
    const failures = [
    ...
    const bridges = learnProtocolBridges(failures);
    ...
    expect(hasAuthFile || hasAuthDb).toBe(true);

    // Both have confidence 1.0 (both appear the same number of times)
    expect(bridges[0].confidence).toBe(1);
  });
});
