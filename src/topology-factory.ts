...
export const ALL_TOPOLOGIES: TopologyName[] = [
 ...
 */
export function createProtocolForTopology(
  type: TopologyName,
  numStates?: number
  ...
  const byAction = new Map<string, { from: Set<string>; to: Set<string> }>();
  ...
  for (const [action, { from, to }] of byAction) {
    ...
    for (const f of from) {
      ...
      for (const [otherAction, otherEntry] of byAction) {
        ...
        if (otherEntry.from.has(f)) { stillNeeded = true; break; }
      }
...
function buildRules(type: TopologyName, n?: number): ProtocolRule[] {
  switch (type) {
...
function buildLinear(n: number): ProtocolRule[] {
...
function buildStar(): ProtocolRule[] {
  return [
...
function buildTree(branchFactor: number, depth: number): ProtocolRule[] {
  const rules: ProtocolRule[] = [];
  rules.push({ from: "INIT", to: "ROOT", action: "enter" });
  let nodeCount = 1;
  const id = (d: number, b: number) => `N${d}_${b}`;
...
function buildLoop(): ProtocolRule[] {
  return [
...
function buildTwoPhaseCommit(): ProtocolRule[] {
  return [
...
function buildAuthBridge(): ProtocolRule[] {
  return [
...
function buildNested(): ProtocolRule[] {
  return [
...
function buildRollback(): ProtocolRule[] {
  return [
...
function buildFanOut(): ProtocolRule[] {
  return [
...
function buildStateless(): ProtocolRule[] {
  return [
