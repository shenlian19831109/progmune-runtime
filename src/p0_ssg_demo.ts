...
import { SSGRejection, FunctionProtocol, rebuildState, validateTransition, checkLedgerConsistency, hashRules, hashLedger, ValidationContext, applyTransitionDelta, findFixPathStatic, explainRejection, rejectionToJSON, findProducer, findConsumer, findViolations, findTransition, listAllStates } from './ssg-validator';
...
function runDemo() {
  ...
  if (!nsInit.has('_global')) nsInit.set('_global', fromFile.initialState || 'INIT');
  const ruleHash = hashRules(rules);

  function createPureContext() {
    let ctx: ValidationContext = { ledger: [], currentState: rebuildState([], nsInit) };
    let transitions: StateTransition[] = [];
    return {
      apply(fn: string) {
        const { valid, transition, rejection } = validateTransition(ctx, fn, transitions.length, rules, nsInit, ruleHash);
      ...
      getTrace() {
        return transitions.map(t => ({
          ...
          rejection: t.valid ? undefined : { fixPath: findFixPathStatic(rules, t.namespace, t.statesBefore[t.namespace] || [], rules.get(t.function)?.pre_states || []) } as any,
        }));
      },
    };
  }
  ...
  for (const fn of legalSequence) {
    ...
    if (!result.valid) {
      console.log(explainRejection(result.rejection!));
      allPassed = false;
      break;
    } else {
  ...
  if (allPassed) console.log('  ✔ 合法序列全部通过 (纯函数验证)\n');
  ...
  for (const fn of badSequence1) {
    ...
    if (!result.valid) {
      ...
      console.log();
      console.log('  📊 结构化报告:');
      console.log('  ' + JSON.stringify(rejectionToJSON(result.rejection!), null, 2).replace(/\n/g, '\n  '));
      break;
    }
    console.log(`  ✅ ${fn} → [${Object.values(result.statesAfter).flat().join(', ')}]`);
  }
  ...
  if (!result3.valid) {
    console.log(explainRejection(result3.rejection!));
  }
  ...
  trace.forEach((node: any, i: number) => {
  ...
  if (!preRead.valid) {
    console.log(explainRejection(preRead.rejection!));
    console.log();
  }
  ...
  let demoLedger: StateTransition[] = [];
  ...
  for (const fn of legalSequence) {
    const { valid, transition, rejection } = validateTransition(ctx, fn, demoLedger.length, rules, nsInit, ruleHash);
  ...
  const rebuilt = rebuildState(demoLedger, nsInit);
  ...
  // Invariant check
  const consistency = checkLedgerConsistency(demoLedger, nsInit);
  ...
  const badConsistency = checkLedgerConsistency([badTransition], nsInit);
