...
function jsonReply(res: http.ServerResponse, data: any, code = 200) {
  res.writeHead(code, {
...
const apiRoutes: Record<string, (url: URL) => any> = {
  "/api/sessions": () => getAllSessions(),
  "/api/genome": () => getFailureGenome(),
  "/api/heatmap": () => getSemanticHeatmap(),
  "/api/antibodies": () => getAntibodyStats(),
  "/api/learned": () => getLearnedPatterns(),
};
...
function handleAPI(req: http.IncomingMessage, res: http.ServerResponse) {
  ...
  if (pathname.startsWith("/api/sessions/")) {
    ...
    if (sessionId) {
      ...
      if (session) return jsonReply(res, session);
