#!/bin/bash
set -e
echo "📦 生成 500 函数大型测试项目..."
PROJECT="test-500"
rm -rf $PROJECT
mkdir -p $PROJECT/services $PROJECT/utils $PROJECT/models $PROJECT/api $PROJECT/tasks $PROJECT/workers
...
# 批量生成填充函数到500（此处使用脚本动态追加）
for i in $(seq 1 30); do
  cat >> $PROJECT/models/data.py << EOF
def func_model_${i}(a: int, b: str) -> Dict: return {"a": a, "b": b}
EOF
done
...
for i in $(seq 1 30); do
  cat >> $PROJECT/utils/helpers.py << EOF
def util_func_${i}(x: int, y: str) -> Tuple[int, str]: return (x, y)
EOF
done
...
for i in $(seq 1 20); do
  cat >> $PROJECT/services/auth.py << EOF
def auth_aux_${i}(x: str) -> str: return x
EOF
done
...
for i in $(seq 1 20); do
  cat >> $PROJECT/services/payment.py << EOF
def pay_aux_${i}(n: float) -> float: return n
EOF
done
...
for i in $(seq 1 20); do
  cat >> $PROJECT/api/endpoints.py << EOF
def api_aux_${i}(p: int) -> int: return p
EOF
done
...
for i in $(seq 1 10); do
  cat >> $PROJECT/tasks/jobs.py << EOF
def job_aux_${i}(x: int) -> int: return x
EOF
done

cat > $PROJECT/workers/handlers.py << 'EOF'
...
for i in $(seq 1 10); do
  cat >> $PROJECT/workers/handlers.py << EOF
def worker_aux_${i}(d: Dict) -> Dict: return d
EOF
done

# 确保所有导入类型正确
for f in $(find $PROJECT -name "*.py"); do
  sed -i '' '1s/^/from typing import Dict, List, Optional, Any, Tuple, Union, Callable\n/' "$f"
done

FCOUNT=$(find $PROJECT -name "*.py" -exec grep "^def " {} \; | wc -l | tr -d ' ')
echo "✅ 函数总数: $FCOUNT"

# 运行压力测试（使用之前的 generate.ts，会读取新项目）
cat > src/generate_500.ts << 'TESTEOF'
...
interface TestResult {
  intent: string; planner: string; duration_ms: number; llm_calls: number; success: boolean; error?: string;
}
...
async function main() {
  ...
  for (const intent of intents) {
    for (const planner of planners) {
      ...
      const valid = actions.length>0 && actions.map((a:any)=>validateAction(a)).every((r:any)=>r.valid);
      if (!valid) {
      ...
      try {
        ...
        success = true;
      } catch(e: any) { error = e.stderr?.toString()||e.toString(); }
...
echo "开始运行压力测试..."
npx ts-node src/generate_500.ts
