...
function computeMaturity(repos, sequences, confidence) {
    if (repos >= 3 && sequences >= 500 && confidence >= 90)
        return "certified";
    if (repos >= 2 && sequences >= 100 && confidence >= 70)
        return "stable";
    if (repos >= 1 && sequences >= 10 && confidence >= 40)
        return "validated";
    return "experimental";
}
// ═══════════════════════════════════════════════════════════════
...
function buildKnowledgeBase() {
    ...
    const byMaturity = { experimental: 0, validated: 0, stable: 0, certified: 0 };
    for (const u of units)
        byMaturity[u.maturity]++;
    ...
    for (const u of units) {
        ...
        if (u.maturity === "stable")
            domainMap[u.domain].stableCount++;
    }
...
// Formatter
// ═══════════════════════════════════════════════════════════════
function formatKnowledgeBase(kb, assetFilter) {
    const C = { r: "\x1b[0m", b: "\x1b[1m", d: "\x1b[2m", g: "\x1b[32m", y: "\x1b[33m", r2: "\x1b[31m", c: "\x1b[36m", m: "\x1b[35m" };
    const l = [];
    const maturityIcon = { experimental: C.r2 + "●" + C.r, validated: C.y + "◉" + C.r, stable: C.g + "★" + C.r, certified: C.m + "⬡" + C.r };
    const maturityBar = {
    ...
    const levels = ["experimental", "validated", "stable", "certified"];
...
// ═══════════════════════════════════════════════════════════════
exports.REGISTRY_PATH = "benchmarks/protocol-knowledge.json";
if (require.main === module) {
    ...
    if (args.includes("--export")) {
        ...
        fs.writeFileSync(exports.REGISTRY_PATH, JSON.stringify(kb, null, 2));
        console.error(`✅ Exported: ${exports.REGISTRY_PATH}`);
    }
