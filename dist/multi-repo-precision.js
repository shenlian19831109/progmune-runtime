...
exports.runMultiRepoPrecision = runMultiRepoPrecision;
exports.formatMultiRepoReportTerminal = formatMultiRepoReportTerminal;
exports.exportMultiRepoReportJSON = exportMultiRepoReportJSON;
...
function ensureRepo(name) {
    ...
    if (fs.existsSync(path.join(dir, ".git"))) {
        ...
        try {
            (0, child_process_1.execSync)(`cd ${dir} && git pull --ff-only 2>/dev/null || true`, { stdio: "ignore", timeout: 30000 });
        }
        catch { /* skip */ }
    }
    else {
        ...
        if (!cfg)
            throw new Error(`No clone URL for: ${name}`);
        console.error(`  ↓ Cloning ${name} (depth=${cfg.depth || 100})...`);
        const parent = path.dirname(dir);
        if (!fs.existsSync(parent))
            fs.mkdirSync(parent, { recursive: true });
        (0, child_process_1.execSync)(`git clone --depth=${cfg.depth || 100} ${cfg.url} ${dir}`, { stdio: "ignore", timeout: 120000 });
    }
    return dir;
}
// ═══════════════════════════════════════════════════════════════
...
function countLinesOfCode(dir) {
    ...
    try {
        ...
        for (const f of files) {
            try {
                count += fs.readFileSync(f, "utf-8").split("\n").length;
            }
            catch { /* skip */ }
        }
    }
...
function extractCallSequences(ir) {
    const sequences = [];
    const funcNames = ir.map((f) => f.name);
    ...
    for (const fn of ir) {
        if (fn.protocol) {
            ...
            if (fn.protocol.pre_states)
                seq.push(...fn.protocol.pre_states);
            seq.push(fn.name);
            if (fn.protocol.post_states)
                seq.push(...fn.protocol.post_states);
            if (seq.length >= 2)
                sequences.push(seq);
        }
    }
...
function runRepoPrecision(target) {
    const errors = [];
    ...
    let ir = [];
    try {
        ir = (0, extract_ir_1.extractIR)(dir);
    }
    catch (e) {
        ...
        if (cleanSeqs.length > 0) {
            ...
            for (const t of PROTOCOL_WEAKNESS_TAXONOMY) {
                for (const seq of sequences) {
                    const hasMatch = t.example_broken.some((step) => seq.some((s) => s.toLowerCase().includes(step.toLowerCase())));
                    if (hasMatch) {
                        cveMatched++;
                        break;
                    }
...
// Report Generator
// ═══════════════════════════════════════════════════════════════
function runMultiRepoPrecision(targets = DEFAULT_TARGETS) {
    const repos = [];
...
// Formatters
// ═══════════════════════════════════════════════════════════════
function formatMultiRepoReportTerminal(report) {
    const lines = [];
...
function exportMultiRepoReportJSON(report, outputPath) {
