...
exports.runPrecision = runPrecision;
exports.formatPrecisionTerminal = formatPrecisionTerminal;
exports.formatPrecisionMarkdown = formatPrecisionMarkdown;
exports.formatMultiRepoTable = formatMultiRepoTable;
...
function runPrecision(repoPath) {
...
function formatPrecisionTerminal(m) {
    const pct = (v) => `${(v * 100).toFixed(0)}%`;
    const color = (v, threshold = 0.7) => v >= threshold ? C.green : v >= 0.5 ? C.yellow : C.red;
    const lines = [];
    lines.push("");
...
function formatPrecisionMarkdown(m) {
    const pct = (v) => `${(v * 100).toFixed(0)}%`;
    const lines = [];
...
// ═══════════════════════════════════════════════════════════════
function formatMultiRepoTable(matrices) {
    const pct = (v) => `${(v * 100).toFixed(0)}%`;
    const lines = [];
    lines.push("");
