...
const accountability_1 = require("./accountability");
const C = {
...
if (cmd === "replay") {
    ...
    try {
        const chain = (0, chain_builder_1.buildProvenanceChain)(sessionId);
    ...
    try {
        const chain = (0, accountability_1.buildAccountabilityChain)(sessionId, {
            author,
...
function resultIcon(r) {
    if (r === "passed" || r === "approved")
        return `${C.green}✓${C.reset}`;
    if (r === "failed")
        return `${C.red}✗${C.reset}`;
    return `${C.yellow}↻${C.reset}`;
}
function formatProvenanceChain(chain) {
    const lines = [];
    lines.push("");
    ...
    for (let i = 0; i < chain.events.length; i++) {
        ...
        lines.push(`  ${String(i).padStart(2, "0")}  ${stepLabel(e.step)} ${resultIcon(e.result)} ${e.artifact.padEnd(28).slice(0, 28)} ${e.hash} ${C.dim}←${prev}${C.reset}`);
        if (e.detail)
...
function formatAccountabilityChain(chain) {
    const lines = [];
    ...
    for (let i = 0; i < chain.events.length; i++) {
        ...
        lines.push(`  ${String(i).padStart(2, "0")} ${icon} ${actorColor}${e.actorLabel.padEnd(38).slice(0, 38)}${C.reset} ${resultIcon(e.result)} ${e.action.slice(0, 30)}`);
