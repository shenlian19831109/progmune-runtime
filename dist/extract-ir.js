...
const AUTH_SKIP_DECORATOR = /^(?:Public|SkipAuth|AllowAnonymous|IsPublic|NoAuth|ApiExcludeEndpoint)$/i;
/** 承载鉴权的装饰器名（NestJS 约定：Guard = 鉴权/授权）。 */
const AUTH_DECORATOR = /^(?:UseGuards|Roles|Permissions|Authorize|RequireAuth|RequiresAuth|Authenticated|UseAuth|CheckPermissions|RequirePermissions|AdminOnly)$/i;
 ...
 */
function authMachineryFromDecorators(decorators) {
    let skip = false;
    let auth = false;
    for (const d of decorators) {
        ...
        if (/^Api/i.test(name))
            continue;
        if (!AUTH_DECORATOR.test(name) && !/guard|auth/i.test(name))
            continue;
        const args = /^@[\w$]*\(([\s\S]*)\)$/.exec(text)?.[1] ?? "";
        if (args.trim() && !GUARD_ARG_AUTH.test(args))
            continue;
        auth = true;
    }
...
const VALIDATOR_DECORATOR_PREFIX = /^(?:Min|Max|Length|Matches|ArrayMinSize|ArrayMaxSize|Validate|Equals|NotEquals|Allow|Contains|NotContains)/;
/** class-validator 的 Is* 族；@IsOptional 只是「可缺省」，不校验 ⇒ 排除 */
const VALIDATOR_DECORATOR_IS = /^Is(?!Optional$)[A-Z]/;
function decoratorIsValidator(text) {
    ...
    for (const sf of project.getSourceFiles()) {
        ...
        for (const cls of sf.getClasses()) {
            ...
            for (const d of cls.getDecorators()) {
                if (decoratorIsValidator(d?.getText?.() ?? "")) {
                    hit = true;
                    break;
                }
            }
...
function bodyHasInputGuard(text) {
    if (!text)
        return false;
    ...
    for (const m of text.matchAll(/\blimits\s*:\s*\{/gi)) {
        ...
        if (/\b(?:fileSize|maxFileSize|files|fields|parts)\b/i.test(tail))
            return true;
    }
 ...
 */
function inputGuardFunctionKeys(project, absRoot) {
    const out = new Set();
    const consider = (name, node, fileRel) => {
        if (!name || !node)
            return;
        if (bodyHasInputGuard(node.getText?.() ?? ""))
            out.add(`${fileRel}|${name}`);
    };
    for (const sf of project.getSourceFiles()) {
        if (sf.getFilePath().includes("node_modules"))
            continue;
        ...
        for (const fn of sf.getFunctions())
            consider(fn.getName(), fn, fileRel);
        for (const cls of sf.getClasses()) {
...
function bodyHasExternalEffect(text) {
    if (!text)
        return false;
 ...
 * key = `${相对路径}|${函数名}`（类方法为 `Class.method`，与 IR 的 f.name 一致）。
 */
function inputEffectFunctionKeys(project, absRoot) {
    const out = new Set();
    const consider = (name, node, fileRel) => {
        if (!name || !node)
            return;
        if (bodyHasExternalEffect(node.getText?.() ?? ""))
            out.add(`${fileRel}|${name}`);
    };
    for (const sf of project.getSourceFiles()) {
        if (sf.getFilePath().includes("node_modules"))
...
function isBaseDirIdent(id) {
    if (BASE_DIR_IDENT_DENY.test(id))
        return false;
    return /^(?:[A-Za-z_$][\w$]*)?(?:base|root|dir)$/i.test(id);
}
const PATH_GUARD_RULES = [
    {
        id: "G-A",
 ...
 */
const PATH_NON_GUARD_NOTE = "N-A path.basename / N-B 单独出现的 join·resolve / N-C 长度或字符数检查 / N-D if (!p) throw —— 均不计入守卫";
/**
 ...
 */
const PATH_GUARD_FN_CALL_RE = /\b(?:assert|ensure|check|validate|verify|sanitize|is)[A-Za-z$]*(?:Safe|Path|Paths|Within|Inside|InDir|Contained|Traversal|Id|Root|Base)[A-Za-z0-9_$]*(?=\s*\()/g;
/** 形态上会命中 G-C 但**确定不是**路径校验的名字（实证反例，逐个加） */
const PATH_GUARD_FN_DENY = /^(?:ensureDir|isDirectory|isDir|assertDir|makeDir|mkdir|checkExists|isValid|isRoot|ensureId|getId)$/i;
function callsPathGuardFn(text) {
    PATH_GUARD_FN_CALL_RE.lastIndex = 0;
    let m;
    while ((m = PATH_GUARD_FN_CALL_RE.exec(text)) !== null) {
        if (!PATH_GUARD_FN_DENY.test(m[0]))
            return true;
    }
    return false;
}
/** 返回命中的守卫规则 id；无校验证据返回 null。 */
function hasPathGuardEvidence(text) {
    for (const r of PATH_GUARD_RULES) {
        if (r.decide ? r.decide(text) : r.re.test(text))
            return r.id;
    }
    if (callsPathGuardFn(text))
        return "G-C";
    return null;
}
function escapeRe(s) {
...
function pathGuardFunctionNames(project) {
    ...
    for (let round = 0; round < 3; round++) {
        ...
        for (const b of bodies) {
            if (b.names.every((n) => guarded.has(n)))
                continue;
            re.lastIndex = 0;
            if (re.test(b.text)) {
...
function isPureShaperExpr(expr, params, shapers, extraIdents) {
    ...
    while ((m = idRe.exec(s)) !== null) {
        ...
        if (params.has(n) || SHAPER_ALLOWED_FREE_IDENTS.has(n))
            continue;
        if (SHAPER_ALLOWED_KEYWORDS.has(n))
            continue; // C4j：运算符关键字不携带数据
 ...
 */
function moduleConstTexts(project) {
    const out = new Map();
    for (const sf of project.getSourceFiles()) {
        for (const vd of sf.getVariableDeclarations()) {
            ...
            if (isLiteral)
                out.set(vd.getName(), init.getText().replace(/^["'`]|["'`]$/g, ""));
        }
    }
    return out;
}
 ...
 */
function propertyNameOf(pa, constTexts) {
    const nn = pa.getNameNode();
    if (ts_morph_1.Node.isIdentifier(nn))
        return nn.getText();
    if (ts_morph_1.Node.isStringLiteral(nn) || ts_morph_1.Node.isNoSubstitutionTemplateLiteral(nn)) {
        return nn.getText().replace(/^["'`]|["'`]$/g, "");
    }
    if (ts_morph_1.Node.isComputedPropertyName(nn)) {
        const e = nn.getExpression();
        // `const KEY = "toPath"; … { [KEY]: (n) => … }` —— KEY 的值必须看得见；
        // `[key()]` 这种算不出来的，宁可整个不收（探针 K2 守的就是这条）。
        if (ts_morph_1.Node.isIdentifier(e))
            return constTexts.get(e.getText()) ?? null;
    }
    return null;
}
/**
 ...
 */
function ownerQualifier(md, constTexts = new Map()) {
...
function moduleQualifier(fn) {
    ...
    for (let guard = 0; guard < 6; guard++) {
        ...
        if (ts_morph_1.Node.isModuleBlock(up)) {
            const decl = up.getParent();
            if (!decl || !ts_morph_1.Node.isModuleDeclaration(decl))
                return null;
        ...
        if (ts_morph_1.Node.isModuleDeclaration(up)) {
            const n = up.getName();
            if (!n)
                return null;
            parts.unshift(n);
        ...
        if (ts_morph_1.Node.isSourceFile(up))
 ...
 */
function unwrapFns(n) {
    let cur = n;
    for (let i = 0; i < 6 && cur; i++) {
        if (ts_morph_1.Node.isParenthesizedExpression(cur)) {
            cur = cur.getExpression();
            continue;
        }
        if (ts_morph_1.Node.isAsExpression(cur)) {
            cur = cur.getExpression();
            continue;
        }
        if (ts_morph_1.Node.isSatisfiesExpression(cur)) {
            cur = cur.getExpression();
            continue;
        }
        if (ts_morph_1.Node.isTypeAssertion(cur)) {
            cur = cur.getExpression();
            continue;
        }
        break;
 ...
 */
function functionLikeOf(init) {
    // 实测：`(() => (n) => n + ".md")()` 里被括号包住的**不止**内层箭头 ——
    // 连被调用的 callee 都是 ParenthesizedExpression。两处都得脱。
    const base = unwrapFns(init);
    if (!base)
        return null;
    if (ts_morph_1.Node.isArrowFunction(base) || ts_morph_1.Node.isFunctionExpression(base))
        return base;
    if (ts_morph_1.Node.isCallExpression(base)) {
        const callee = unwrapFns(base.getExpression());
        if (!ts_morph_1.Node.isArrowFunction(callee) && !ts_morph_1.Node.isFunctionExpression(callee))
            return null;
        const b = callee.getBody();
        if (!ts_morph_1.Node.isBlock(b)) {
            // 简洁体：`(() => (n) => n + ".md")()` —— 括号包着也算（unparen）
            const e = unwrapFns(b);
            return e && (ts_morph_1.Node.isArrowFunction(e) || ts_morph_1.Node.isFunctionExpression(e)) ? e : null;
        }
        for (const rs of b.getDescendantsOfKind(ts.SyntaxKind.ReturnStatement)) {
            const e = unwrapFns(rs.getExpression());
            if (e && (ts_morph_1.Node.isArrowFunction(e) || ts_morph_1.Node.isFunctionExpression(e)))
                return e;
        }
    }
    return null;
 ...
 */
function curryChainOf(outerParams, body) {
    const ret = returnedValueOf(body);
    if (!ret)
        return null;
    if (!ts_morph_1.Node.isArrowFunction(ret) && !ts_morph_1.Node.isFunctionExpression(ret))
        return null;
    const hops = [];
    let cur = ret;
    for (let depth = 0; depth < 4; depth++) {
        if (!cur)
            break;
        if (!ts_morph_1.Node.isArrowFunction(cur) && !ts_morph_1.Node.isFunctionExpression(cur))
            return null;
        ...
        if (!next) {
            ...
            if (hops[hops.length - 1].params.length === 0)
                return null;
 ...
 */
function currySpecsOf(outerParams, body) {
    const ret = returnedValueOf(body);
    if (!ret)
        return [];
    if (!ts_morph_1.Node.isObjectLiteralExpression(ret)) {
    ...
    for (const prop of ret.getProperties()) {
        ...
        if (ts_morph_1.Node.isPropertyAssignment(prop)) {
            ...
            if (inner && (ts_morph_1.Node.isArrowFunction(inner) || ts_morph_1.Node.isFunctionExpression(inner))) {
                fn = inner;
            }
        }
        else if (ts_morph_1.Node.isMethodDeclaration(prop)) {
            name = prop.getName();
            fn = prop;
        }
        if (!fn || !name || !/^[A-Za-z_$][\w$]*$/.test(name))
            continue;
...
function returnedValueOf(body) {
    if (!ts_morph_1.Node.isBlock(body))
        return unwrapFns(body);
...
function returnedFunctionOf(body) {
    if (!ts_morph_1.Node.isBlock(body)) {
 ...
 */
function expandCurried(text, curried) {
    if (curried.size === 0)
        return text;
    let cur = text;
...
function expandCurriedOnce(text, curried) {
        ...
        while ((m = re.exec(text)) !== null) {
            ...
            for (const cs of specs) {
                ...
                args0.forEach((a, i) => {
                    ...
                    if (p !== undefined)
                        map.set(p, stripOuterParens(a.trim()));
                });
...
function substituteParams(expr, litArgs) {
    let out = "";
    let quote = "";
    let i = 0;
    while (i < expr.length) {
 ...
 */
function foldDecidableTernary(expr, depth = 0) {
    if (depth > 4)
        return expr;
 ...
 * 知道这一层没做事，好继续试三元/短路）。
 */
function foldTemplateSubsts(expr, depth) {
    if (!expr.includes("`"))
        return expr;
    let out = "";
    let changed = false;
 ...
 */
function mergeShapers(infos) {
    if (infos.length === 0)
        return { params: [], deps: new Set(), rets: [], restAt: -1 };
    ...
    for (const si of infos) {
        if (si.params.length !== params.length || si.params.some((p, i) => p !== params[i]))
            sameShape = false;
        si.params.forEach((p, i) => { if (i < flags.length && si.deps.has(p))
            flags[i] = true; });
    }
 ...
 */
function defaultImportLocalNames(project, target) {
    const out = new Set();
    const targetPath = target.getFilePath();
    for (const sf of project.getSourceFiles()) {
        if (sf.getFilePath() === targetPath)
            continue;
        for (const imp of sf.getImportDeclarations()) {
            ...
            if (!resolved || resolved.getFilePath() !== targetPath)
                continue;
...
function pureShaperFunctionNames(project) {
     ...
     */
    const noteUnaccepted = (key, group) => {
        if (group === null)
            return;
    ...
    const consider = (key, name, group, paramNodes, restAt, curry, declText, bodyNode) => {
        const paramList = paramNodes.filter(Boolean);
        const params = new Set(paramList);
        if (params.size === 0)
            return noteUnaccepted(key, group); // 判据①
        sinkRe.lastIndex = 0;
        if (sinkRe.test(declText))
            return noteUnaccepted(key, group); // ②
        ...
        if (rets.length === 0 && curry.length === 0)
            return noteUnaccepted(key, group); // ③
        let pub = name;
        if (pub !== null) {
            if (takenNames.has(pub))
                pub = null; // 同名重复登记：先到先得，后者降到资格凭证
            else
                takenNames.add(pub);
        }
    ...
    for (const sf of project.getSourceFiles()) {
        ...
        for (const fn of sf.getDescendantsOfKind(ts.SyntaxKind.FunctionDeclaration)) {
            ...
            if (!ts_morph_1.Node.isSourceFile(parent) && !ts_morph_1.Node.isModuleBlock(parent))
                continue;
            ...
            if (!ts_morph_1.Node.isSourceFile(parent) && owner === null)
                continue;
            // namespace 内：`P.toPath(k)` 是主要写法。裸名同时登记是安全的 ——
            // takenNames 先到先得，一个名字只会落到一处定义上（另一处会降级为资格凭证）。
            if (owner !== null) {
                consider(`fn:${owner}.${name}`, `${owner}.${name}`, null, paramNodes, restAt, curry, fn.getText(), body);
            }
        ...
        for (const vd of sf.getVariableDeclarations()) {
            ...
            const paramNodes = ps.flatMap(paramNamesOf);
            consider(`var:${vd.getName()}`, vd.getName(), null, paramNodes, restIndexOf(ps), currySpecsOf(paramNodes, body), init.getText(), // IIFE 时含整条调用 ⇒ sink 检查仍覆盖得到
            body);
        }
    }
    ...
    const considerMember = (bare, owner, paramNodes, restAt, curry, declText, body) => {
        memberSeq += 1;
        consider(`mb:${memberSeq}`, owner !== null ? `${owner}.${bare}` : null, bare, paramNodes, restAt, curry, declText, body);
    };
    for (const sf of project.getSourceFiles()) {
        // (c) 对象字面量方法 / 类方法
        for (const md of sf.getDescendantsOfKind(ts.SyntaxKind.MethodDeclaration)) {
            ...
            considerMember(bare, ownerQualifier(md, constTexts), pns, restIndexOf(ps), currySpecsOf(pns, body), md.getText(), body);
        }
        ...
        for (const pa of sf.getDescendantsOfKind(ts.SyntaxKind.PropertyAssignment)) {
            ...
            const ps = fn.getParameters();
            const pns = ps.flatMap(paramNamesOf);
            considerMember(bare, ownerQualifier(pa, constTexts), pns, restIndexOf(ps), currySpecsOf(pns, body), rawInit.getText(), body);
        }
        // (e) 后挂上去的属性箭头 `Util.toPath = (n) => …`（monkey patch / 渐进导出）
        for (const be of sf.getDescendantsOfKind(ts.SyntaxKind.BinaryExpression)) {
            if (be.getOperatorToken().getKind() !== ts.SyntaxKind.EqualsToken)
                continue;
            const left = be.getLeft();
            const right = unwrapFns(be.getRight());
            if (!ts_morph_1.Node.isPropertyAccessExpression(left))
                continue;
            if (!right || (!ts_morph_1.Node.isArrowFunction(right) && !ts_morph_1.Node.isFunctionExpression(right)))
                continue;
            const body = right.getBody();
            if (!body)
                continue;
            const qname = left.getText();
            // 只收 `Name.member` 这种纯静态链（`this.x` / `a[b].x` 一律不认）
            if (!/^[A-Za-z_$][\w$]*(?:\.[\w$]+)+$/.test(qname))
                continue;
            const bare = qname.split(".").pop();
            const ps = right.getParameters();
            const pns = ps.flatMap(paramNamesOf);
            considerMember(bare, qname.slice(0, -(bare.length + 1)), pns, restIndexOf(ps), currySpecsOf(pns, body), right.getText(), body);
        }
        ...
        for (const ga of sf.getDescendantsOfKind(ts.SyntaxKind.GetAccessor)) {
            ...
            if (!inner)
                continue; // getter 直接返回拼接（无形参）⇒ 判据①本就不成立
            ...
            const pns = ps.flatMap(paramNamesOf);
            considerMember(bare, ownerQualifier(ga, constTexts), pns, restIndexOf(ps), currySpecsOf(pns, innerBody), ga.getText(), innerBody);
        }
        ...
        for (const ea of sf.getDescendantsOfKind(ts.SyntaxKind.ExportAssignment)) {
            ...
            if (!fn || (!ts_morph_1.Node.isArrowFunction(fn) && !ts_morph_1.Node.isFunctionExpression(fn)))
                continue;
            ...
            if (localNames.size === 0)
                continue; // 没人默认导入 ⇒ 名字无从谈起
            const ps = fn.getParameters();
            const pns = ps.flatMap(paramNamesOf);
            const curry = currySpecsOf(pns, fnBody);
            for (const ln of localNames) {
                consider(`def:${sf.getFilePath()}:${ln}`, ln, null, pns, restIndexOf(ps), curry, ea.getText(), fnBody);
            }
        }
    }
    ...
    // 按裸名位去匹配」。
    const prelimMember = new Set(groups.keys());
    const publish = (name, si) => {
        if (names.has(name))
            return;
        names.add(name);
        info.set(name, si);
    ...
    const pureCurried = new Map();
    for (let round = 0; round < 2; round++) {
        let added = false;
        for (const c of cands) {
            if (acceptedKeys.has(c.key))
                continue;
            let si;
            if (c.curry.length > 0) {
                ...
                const pure = c.curry.filter((spec) => spec.rets.every((e) => isPureShaperExpr(e, new Set([...c.params, ...spec.allParams]), names, extraIdents)));
                if (pure.length === 0)
                    continue;
                ...
                if (!c.rets.every((e) => isPureShaperExpr(e, c.params, names, extraIdents)))
                    continue;
                acceptedKeys.add(c.key);
                si = {
                    params: c.paramList,
    ...
    for (const [bare, keys] of groups) {
        if (!keys.every((k) => acceptedKeys.has(k)))
            continue;
        memberNames.add(bare);
        publish(bare, mergeShapers(keys.map((k) => keyInfo.get(k)).filter(Boolean)));
    }
 ...
 */
const TS_PATH_SHAPER_RE = /(?:path\s*\.\s*(?:posix|win32)\s*\.\s*|path\s*\.\s*)?\b(?:join|resolve|normalize|basename|dirname|extname|relative|format|toNamespacedPath)\s*\(|\.\s*(?:trim|trimStart|trimEnd|replace|replaceAll|toLowerCase|toUpperCase|toString|slice|substring|substr|padStart|padEnd|concat|normalize)\s*\(/;
...
function taintedViaShaper(text, tainted, shapers) {
    ...
    for (let depth = 0; depth < 3; depth++) {
        ...
        for (const line of text.split("\n")) {
            ...
            const shaped = TS_PATH_SHAPER_RE.test(expanded) ||
            ...
            // 返回值依不依赖实参是两件事。
            const evidence = shaperInfo && memberNames
    ...
    if (hasRequestRootedExpr(text)) {
        ...
        if (!selfGuarded) {
            if (hasTaintedSinkCall(text, tainted, sanitizedRe)) {
                ...
                const directCallRe = new RegExp(`(?:^|[^\\w$.])([\\w$]+)\\s*\\(([\\s\\S]{0,250}?)\\)`, "g");
                const taint = taintPattern(tainted);
                if (taint) {
...
function _extractSingleProject(absRoot, tsconfigPath) {
    ...
    if (!project.getSourceFiles().some((sf) => !sf.getFilePath().includes("node_modules"))) {
        console.error(`⚠️  [extract-ir] ${absRoot}：tsconfig 未纳入任何源文件（solution 风格 / 引用缺失）→ 回退为全目录 glob`);
        project.addSourceFilesAtPaths(path.join(absRoot, "**/*.ts"));
    ...
    for (const sf of project.getSourceFiles()) {
        ...
        for (const cls of sf.getClasses()) {
            ...
            for (const m of cls.getMethods()) {
                ...
                mCalls.push(...computeMarkerCalls(mText, mParams.map((p) => p.getName()), sinkParams, onMethodHit, guardFns, directGuardFns, shapers));
    ...
    for (const f of funcs) {
        if (!f.params || f.params.length === 0)
            continue;
        const hit = f.params.some((p) => p.type && validatedDtoClasses.has(p.type));
        if (!hit)
            continue;
        f.calls = f.calls || [];
        if (!f.calls.includes(INPUT_SCHEMA_MARKER))
            f.calls.push(INPUT_SCHEMA_MARKER);
    }
    ...
    for (const f of funcs) {
        if (!inputGuardKeys.has(`${f.file}|${f.name}`))
            continue;
        f.calls = f.calls || [];
        if (!f.calls.includes(INPUT_GUARD_MARKER))
            f.calls.push(INPUT_GUARD_MARKER);
    }
    // ── E5（2026-09-23）：参数流向外部副作用的证据 ⇒ 注入 __progmune_input_effect__
    ...
    for (const f of funcs) {
        if (!inputEffectKeys.has(`${f.file}|${f.name}`))
            continue;
        f.calls = f.calls || [];
        if (!f.calls.includes(INPUT_EFFECT_MARKER))
            f.calls.push(INPUT_EFFECT_MARKER);
    }
    // ═══════════════════════════════════════════════════════════════
