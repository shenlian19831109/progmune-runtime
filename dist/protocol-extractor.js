...
exports.extractCallPairs = extractCallPairs;
exports.inferProtocolRules = inferProtocolRules;
exports.extractProtocol = extractProtocol;
exports.extractProtocolFromFiles = extractProtocolFromFiles;
exports.rulesToAnnotationMap = rulesToAnnotationMap;
exports.printExtractionReport = printExtractionReport;
 ...
 */
function extractCallPairs(source, file = "<input>") {
    const pairs = [];
 ...
 */
function inferProtocolRules(pairs, minFrequency = 3) {
        ...
        if (isCloser) {
            ...
            for (const [otherFn, otherStates] of fnStates) {
                if (openingKeywords.some(kw => otherFn.includes(kw))) {
                    const sharedState = [...otherStates.produced].find(s => states.required.has(s));
                    if (sharedState) {
...
function inferStateName(fnA, fnB) {
 ...
 */
function extractProtocol(sources, protocolName = "ExtractedProtocol", minFrequency = 3) {
    const allPairs = [];
    const sourceFiles = [];
 ...
 */
function extractProtocolFromFiles(filePaths, protocolName = "ExtractedProtocol", minFrequency = 3) {
 ...
 */
function rulesToAnnotationMap(rules) {
    const map = new Map();
    for (const r of rules) {
        map.set(r.function, {
