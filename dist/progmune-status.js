...
function main() {
    ...
    try {
        ...
        if (m && m.totalScans > 0) {
            ...
            const stableList = kb.units.filter((u) => u.maturity === "stable");
            const totalDebt = stableList.reduce((s, u) => {
    ...
    try {
        ...
        const top3 = g.nodes.map((n) => ({ name: n.name, t: (g.inDegree[n.id] || 0) + (g.outDegree[n.id] || 0) })).sort((a, b) => b.t - a.t).slice(0, 3);
        console.log(`\n  ${C.b}Knowledge Network:${C.r} ${C.d}${g.nodes.length} nodes, ${g.edges.length} edges · Top: ${top3.map((r) => r.name + "(" + r.t + ")").join(", ")}${C.r}`);
    }
