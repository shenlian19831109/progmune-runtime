...
exports.generateFullChangelog = generateFullChangelog;
...
function scanRepo(repoPath) {
    ...
    const matchedProtocols = new Set();
    const unmatched = [];
    ...
    for (const proto of matchedProtocols) {
        const matchingSeqs = sequences.filter((s) => {
            const r = (0, protocol_detector_1.validateProtocolState)(s.calls || []);
            return r.matchedProtocols.includes(proto);
        });
        existingMatches.push({
            protocol: proto,
            sequenceCount: matchingSeqs.length,
            examples: matchingSeqs.slice(0, 3).map((s) => (s.calls || []).slice(0, 5)),
        });
    }
...
function generateProposals(unmatched, kb, repoName) {
    const candidates = [];
...
// Step 2: Propose
// ═══════════════════════════════════════════════════════════════
function proposeUpdate(repoPath) {
    ...
    for (const cand of scan.proposalCandidates) {
        ...
        if (!asset || cand.suggestedAction !== "extend")
            continue;
        changes.push({
            assetId: asset.id,
            assetName: asset.name,
...
// Step 3: Changelog
// ═══════════════════════════════════════════════════════════════
function generateChangelog(changes, repo) {
    const lines = [];
...
function formatUnitEvolution(unit) {
    const l = [];
...
if (require.main === module) {
    ...
    if (cmd === "scan") {
        ...
        try {
            ...
            for (const m of scan.existingMatches) {
                ...
                if (m.examples.length > 0)
                    console.log(`      ${C.d}${m.examples[0].slice(0, 4).join(" → ")}${C.r}`);
            }
        ...
        const v2 = args[3];
        if (!unitId || !v1 || !v2) {
            console.error("Usage: diff <unitId> <v1> <v2>");
            process.exit(1);
        }
        const kb = (0, protocol_knowledge_1.buildKnowledgeBase)();
        ...
        const ver1 = unit.versionHistory.find((v) => v.version === v1);
        const ver2 = unit.versionHistory.find((v) => v.version === v2);
        if (!ver1 || !ver2) {
        ...
        console.log(`  Repos:         ${ver1.validatedRepos.length} → ${ver2.validatedRepos.length}  ${ver2.validatedRepos.filter((r) => !ver1.validatedRepos.includes(r)).length > 0 ? C.g + "+ " + ver2.validatedRepos.filter((r) => !ver1.validatedRepos.includes(r)).join(", ") + C.r : ""}`);
        ...
        const unitId = args[1];
        if (!unitId) {
            console.error("Usage: benchmark <unitId>");
            process.exit(1);
        }
        const kb = (0, protocol_knowledge_1.buildKnowledgeBase)();
        const unit = kb.units.find(u => u.id === unitId || u.name === unitId);
        ...
        if (unitFilter) {
            ...
            if (!unit) {
                console.error(`Unit not found: ${unitFilter}`);
                process.exit(1);
            }
