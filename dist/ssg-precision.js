...
function validateSequenceWithSSG(sequence, rules, nsInit = new Map([["_global", "INIT"]])) {
    ...
    for (let i = 0; i < sequence.length; i++) {
        ...
        const states = nsStates.get(ns) || new Set(["INIT"]);
        // Check preconditions
        ...
        // Apply state changes
        if (rule.invalidate)
            rule.invalidate.forEach(s => states.delete(s));
    ...
    for (const [ns, st] of nsStates) {
        ...
        for (const s of st) {
            ...
            for (const fn of sequence) {
                ...
                if (r && r.post_states.includes(s)) {
                    producedInSequence = true;
                    break;
                }
            }
            if (!producedInSequence)
                continue;
 ...
 */
function checkOrderViolations(sequence, rules, nsInit) {
    const violations = [];
    // Build must-precede graph from rules
    for (const [fnA, ruleA] of rules) {
        if (ruleA.post_states.length === 0)
            continue;
 ...
 */
function discoverRulesFromSequences(sequences) {
 ...
 */
function runSSGPrecisionBenchmark(sequences, labels) {
