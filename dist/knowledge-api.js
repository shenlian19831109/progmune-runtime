...
const server = http.createServer((req, res) => {
    ...
    if (url === "/knowledge/network") {
        const g = (0, knowledge_graph_1.buildKnowledgeGraph)();
        json(res, { nodes: g.nodes.length, edges: g.edges.length, adjacency: g.adjacency, topNodes: g.nodes.map(n => ({ id: n.id, name: n.name, domain: n.domain, maturity: n.maturity, inDegree: g.inDegree[n.id] || 0, outDegree: g.outDegree[n.id] || 0 })).sort((a, b) => (b.inDegree + b.outDegree) - (a.inDegree + a.outDegree)) });
        return;
    }
        ...
        if (fs.existsSync("benchmarks")) {
            for (const f of fs.readdirSync("benchmarks")) {
                if (f.startsWith("kb-package-") && f.endsWith(".json")) {
                    try {
                        pkgs.push(JSON.parse(fs.readFileSync(`benchmarks/${f}`, "utf-8")));
                    }
                    catch { }
                }
            }
    ...
    if (url.startsWith("/knowledge/packages/")) {
        ...
        if (!fs.existsSync(pkgFile)) {
            json(res, { error: "Package not found" }, 404);
            return;
        }
