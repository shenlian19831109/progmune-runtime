...
function classifyFP(calls, funcName, reason) {
    ...
    const resourceVios = (0, resource_detector_1.detectResourceViolations)(calls);
...
function classifyFN(calls, funcName, reason) {
...
function analyze(repoPath) {
    ...
    const fpCases = details.filter((d) => d.expected === "clean" && d.detected === "violation");
    const fnCases = details.filter((d) => d.expected === "violation" && d.detected === "clean");
    console.error(`\nAnalyzing ${report.total} samples: ${report.fp} FP, ${report.fn} FN`);
    // Classify FPs
    const fpByCategory = new Map();
    for (const d of fpCases) {
        const cat = classifyFP(d.calls || [], d.calls?.[0] || "", d.reason || "");
        if (!fpByCategory.has(cat))
            fpByCategory.set(cat, []);
        fpByCategory.get(cat).push(d);
    }
    // Classify FNs
    const fnByCategory = new Map();
    for (const d of fnCases) {
        const cat = classifyFN(d.calls || [], d.calls?.[0] || "", d.reason || "");
        if (!fnByCategory.has(cat))
            fnByCategory.set(cat, []);
