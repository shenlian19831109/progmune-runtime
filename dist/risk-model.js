...
// ═══════════════════════════════════════════════════════════════
function assessRisk(calls, protocolName) {
    const patterns = exports.DETECTION_PATTERNS.filter(p => {
        if (protocolName && p.protocolName !== protocolName)
            return false;
    ...
    let unitId = "", unitVersion = "", unitRfc;
    try {
        const { buildKnowledgeBase } = require("./protocol-knowledge");
        const kb = buildKnowledgeBase();
        const unit = kb.units.find((u) => u.name === patterns[0].protocolName);
        if (unit) {
            unitId = unit.id;
            unitVersion = unit.currentVersion;
            unitRfc = unit.rfcReference;
        }
    }
