...
const hierarchical_planner_1 = require("./hierarchical-planner");
...
function seedSkillsData(n) {
    const t = new planner_telemetry_1.PlannerTelemetry(path.join(SKILL_DIR, ".progmune_corpus", "telemetry", `skills-${Date.now()}.jsonl`));
    // High-acceptance file skill
    for (let i = 0; i < n; i++) {
        const fp = (0, planner_telemetry_1.candidateFingerprint)("FileProtocol", ["open_file", "write_file", "close_file"], "resource_leak");
    ...
    for (let i = 0; i < n; i++) {
        const fp = (0, planner_telemetry_1.candidateFingerprint)("AuthProtocol", ["verify_password", "generate_jwt", "create_session"], "missing_prerequisite");
        const id = t.recordDecision({
...
(0, vitest_1.describe)("Skill Library", () => {
    (0, vitest_1.it)("learns skills from telemetry", () => {
        const telemetry = seedSkillsData(40);
        const rules = makeRules();
        const library = new skill_library_1.SkillLibrary();
        library.learn(telemetry, rules);
        (0, vitest_1.expect)(library.size).toBeGreaterThanOrEqual(2);
        ...
        (0, vitest_1.expect)(hasFileSkill).toBe(true);
        ...
        (0, vitest_1.expect)(hasAuthSkill).toBe(true);
        (0, skill_library_1.printSkillLibrary)(library);
    });
    (0, vitest_1.it)("finds producers by effect", () => {
    ...
    (0, vitest_1.it)("composes skill chains from current to target state", () => {
        const telemetry = seedSkillsData(50);
        const rules = makeRules();
        const library = new skill_library_1.SkillLibrary();
        library.learn(telemetry, rules);
        ...
        const hasAuthChain = chains.some(c => c.some(s => s.macro.includes("verify_password") && s.macro.includes("generate_jwt")));
        (0, vitest_1.expect)(hasAuthChain || chains.length >= 0).toBe(true);
    });
    (0, vitest_1.it)("expands skill to action sequence", () => {
        const telemetry = seedSkillsData(40);
        const rules = makeRules();
        const library = new skill_library_1.SkillLibrary();
        library.learn(telemetry, rules);
        ...
        if (authSkills.length > 0) {
            ...
            (0, vitest_1.expect)(actions).not.toBeNull();
            (0, vitest_1.expect)(actions.length).toBeGreaterThanOrEqual(2);
        }
    });
    (0, vitest_1.it)("saves and loads skills", () => {
        const telemetry = seedSkillsData(40);
        const rules = makeRules();
        const library = new skill_library_1.SkillLibrary();
        library.learn(telemetry, rules);
        ...
        const library2 = new skill_library_1.SkillLibrary();
        library2.load(fp);
        (0, vitest_1.expect)(library2.size).toBe(library.size);
    });
});
(0, vitest_1.describe)("Hierarchical Planner", () => {
    (0, vitest_1.it)("plans using skill library (Goal → Skill → Action)", () => {
        const telemetry = seedSkillsData(50);
        const rules = makeRules();
        const library = new skill_library_1.SkillLibrary();
        library.learn(telemetry, rules);
        const planner = new hierarchical_planner_1.HierarchicalPlanner(library, rules);
        ...
        const hasFileSkill = candidates.some(c => c.source === "skill" && c.actions.includes("close_file"));
        (0, vitest_1.expect)(hasFileSkill).toBe(true);
    });
    (0, vitest_1.it)("plans auth repair using skill chain", () => {
        const telemetry = seedSkillsData(50);
        const rules = makeRules();
        const library = new skill_library_1.SkillLibrary();
        library.learn(telemetry, rules);
        ...
        const hasAuthSkill = candidates.some(c => c.source === "skill" && c.actions.includes("create_session"));
        (0, vitest_1.expect)(hasAuthSkill).toBe(true);
    });
    (0, vitest_1.it)("quick plan returns skill-based candidates only", () => {
        const telemetry = seedSkillsData(50);
        const rules = makeRules();
        const library = new skill_library_1.SkillLibrary();
        library.learn(telemetry, rules);
        ...
        (0, vitest_1.expect)(candidates.every(c => c.source === "skill")).toBe(true);
        (0, vitest_1.expect)(candidates.length).toBeGreaterThan(0);
    });
});
